...
//   scanRoomsPrint();        // pretty-print full results JSON when complete
//
// Console (one-tick, visible-only; no observer):
//   scanRoomsNow('E3N46');   // scans currently visible rooms only and prints a summary
//
// Notes:
// - You can only analyze rooms visible this tick (present in Game.rooms). Visibility requires your presence or an Observer. 【1】
// - Target set is built using Game.map.describeExits and filtered by Game.map.getRoomLinearDistance(center, rn, true) <= 10. 【2】// - Standard queries use Room.find with FIND_* constants and filters. 【3】
...
(function installRoomObserver() {
  ...
  function bfsRoomsWithin(center, radius) {
    ...
    while (q.length > 0) {
      ...
      var exits = Game.map.describeExits(cur.name) || {}; // {1:'',3:'',5:'',7:''} when connected【2】
  ...
  function getDistance(a, b) {
    return Game.map.getRoomLinearDistance(a, b, true); // continuous map【2】
  }
  ...
  function analyzeRoom(center, roomName) {
    ...
    if (!room) return null; // not visible this tick【1】
    ...
    if (typeof FIND_POWER_CREEPS !== 'undefined') {
      ...
      var pcNames = Object.keys(Game.powerCreeps || {}); // accessible always【1】
  ...
  function runOneTick() {
    ...
    for (var o = 0; o < observers.length; o++) {
      ...
      if (!target) break;
      observers[o].observeRoom(target); // will be visible next tick
  ...
  global.scanRoomsStart = function(centerRoomName) {
    ...
    var radiusRooms = bfsRoomsWithin(centerRoomName, RADIUS); // via exits graph【2】
    var filtered = [];
    for (var i = 0; i < radiusRooms.length; i++) {
      var rn = radiusRooms[i];
      if (getDistance(centerRoomName, rn) <= RADIUS) filtered.push(rn); // linear distance guard【2】
    }
  ...
  global.scanRoomsPrint = function() {
    var state = getState();
    return JSON.stringify({
      center: state.center,
      radius: RADIUS,
      scannedRooms: state.results.scannedRooms,
  ...
  global.scanRoomsStop = function() {
    ...
    state.active = false;
    state.pending = {};
    return 'roomObserver: scan canceled.';
  };
  ...
  global.scanRoomsNow = function(centerRoomName) {
    var visibleRooms = Object.keys(Game.rooms); // only visible rooms are accessible this tick【1】
