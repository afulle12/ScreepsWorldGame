 ...
 *
 *   maintScan()
 *     Print energy/tick decay costs for all visible rooms, plus the workforce
 *     body cost (snapshot value and per-day replacement cost based on each
 *     creep's TTL) for creeps owned by each room's controller owner.
 ...
 *
 * ═══════════════════════════════════════════════════════════════════
 * CONSOLE COMMANDS — PLAYER STATUS BOARD
 * ═══════════════════════════════════════════════════════════════════
 *
 *   status('PlayerName')     (the global lives in statusReport.js)
 *     Observer-snapshot every registry room the player owns — one room
 *     per observer per tick via the scheduler at one-shot priority —
 *     then print a status()-style board: per-room energy, storage,
 *     creep census emoji, RCL % + ETA, threat flags. RCL ETA appears
 *     from the second scan onward (needs two anchors >=100t apart).
 *     Re-run while a scan is active to see progress. Plain status()
 *     (no argument) still prints the colony report.
 *
 * ═══════════════════════════════════════════════════════════════════
 ...
 *   heap.roomEffProfile          Active 100-tick efficiency profiles (memoryManager.heap; lost on global reset)
 ...
 *   Memory.playerStatusScan      Active status('Player') scan state
 *   Memory.playerStatusAnchors   Per-player RCL progress anchors for ETA
 *
 * ═══════════════════════════════════════════════════════════════════
 */
...
const INTEL_EFF_MEM_KEY     = 'roomEffProfile'; // lives in memoryManager.heap, not Memory
...
function _bpTTL(body) {
    const len = body && body.length ? body.length : 0;
    return Math.max(1, CREEP_LIFE_TIME + len * 150);
}
...
const MON_SCHEMA_VERSION = 2;    // sparse per-room snapshots
...
const ALERT_FILTER = {
    ALLY: [
        ...
        'SPAWNS_DRAINED', 'TOWERS_EMPTY', 'SAFE_MODE', 'NEW_ROOM',
        'NUKER_READY', 'NUKE_INCOMING'
    ],
    NEUTRAL: [
        'RCL_UPGRADED', 'RCL_DOWNGRADED', 'ROOM_LOST', 'NO_CREEPS',
        'SPAWNS_DRAINED', 'TOWERS_EMPTY', 'SAFE_MODE', 'NEW_ROOM',
        'NUKER_BUILT', 'NUKER_FILLING_G', 'NUKER_FILLING_E',
        'NUKER_READY', 'NUKE_INCOMING', 'MILITARY_CREEPS', 'BOOSTED_CREEPS'
    ],
...
function getMarketBuyPrice(resource) {
    ...
    try {
        const orders=util.marketOrders(resource, ORDER_SELL);
        if (!orders||orders.length===0) return 0;
        return Math.min(...orders.map(o=>o.price));
    } catch(e) { return 0; }
}

/** 48h volume-weighted average price via marketPricing (used by roomIntel scoring) */
function getMarketHistoryPrice(resourceType) {
    try {
        const avg = require('marketPricing').getAvg48h(resourceType);
...
function _mtCreepCost(room) {
    const owner=room.controller&&room.controller.owner;
    if (!owner||!owner.username) return null;
    const ownerName=owner.username;
    let liveCost=0,spawningCost=0,liveCount=0,spawningCount=0,perDay=0;
    const countedNames={};
    function add(body, isSpawning) {
        const cost=_bpCostOf(body);
        const ttl=_bpTTL(body);
        if (isSpawning) { spawningCost+=cost; spawningCount++; }
        else { liveCost+=cost; liveCount++; }
        perDay+=cost*(TICKS_PER_DAY/ttl);
    }
    for (const c of room.find(FIND_CREEPS)) {
        if (!c.owner||c.owner.username!==ownerName) continue;
        add(c.body, !!c.spawning);
        countedNames[c.name]=true;
    }
    for (const s of room.find(FIND_STRUCTURES,{filter:x=>x.structureType===STRUCTURE_SPAWN&&x.owner&&x.owner.username===ownerName&&x.spawning&&x.spawning.body&&!countedNames[x.spawning.name]})) {
        add(s.spawning.body, true);
    }
    return {owner:ownerName,liveCost,spawningCost,liveCount,spawningCount,total:liveCost+spawningCost,perDay};
}
...
function _mtAttachCreepCost(summary, roomName, room) {
    const cc=_mtCreepCost(room);
    if (!summary.rooms[roomName]) summary.rooms[roomName]={totalEptCreep:0,totalEptTower:0,towerExact:false,count:0,types:{}};
    summary.rooms[roomName].creepCost=cc;
    if (!cc) return;
    summary.totalCreepCost+=cc.total;
    summary.totalCreepPerDay+=cc.perDay;
    summary.totalCreepCount+=cc.liveCount;
    summary.totalSpawningCreeps+=cc.spawningCount;
}
...
function maintScan() {
    const summary={totalEptCreep:0,totalEptTower:0,totalCreepCost:0,totalCreepPerDay:0,totalCreepCount:0,totalSpawningCreeps:0,rooms:{}};
    for (const rn in Game.rooms) { const room=Game.rooms[rn]; if(room) { _mtScanRoom(room,rn,(rn,tk,ec,et,te)=>_mtAdd(summary,rn,tk,ec,et,te)); _mtAttachCreepCost(summary,rn,room); } }
    return _mtRound(summary);
}

function _mtPrint(data) {
    console.log('[maint-scan] Total — creep: '+data.totalEptCreep+' en/tick ('+dailyCost(data.totalEptCreep)+'/day) | tower: '+data.totalEptTower+' en/tick ('+dailyCost(data.totalEptTower)+'/day) | creep workforce: '+fmtE(data.totalCreepCost||0)+' en ('+(data.totalCreepCount||0)+' live, '+(data.totalSpawningCreeps||0)+' spawning) | '+fmtE(data.totalCreepPerDay||0)+' en/day');
    ...
    for (const r of rooms) {
        ...
        const cc=r.info.creepCost;
        const cLabel=cc?' | creep workforce '+fmtE(cc.total)+' en ('+cc.liveCount+' live + '+cc.spawningCount+' spawning, owner '+cc.owner+') | '+fmtE(cc.perDay)+' en/day':'';
        console.log('  - '+r.name+': creep '+r.info.totalEptCreep+' ('+dailyCost(r.info.totalEptCreep)+'/day) | '+tLabel+' '+r.info.totalEptTower+' ('+dailyCost(r.info.totalEptTower)+'/day) en/tick (structures '+r.info.count+')'+cLabel);
...
function maintPrint(filterRoom) {
    if (filterRoom) {
        ...
        _mtAttachCreepCost(summary,filterRoom,room);
...
function _itStartProfile(roomName, room) {
    ...
    if(heap[INTEL_EFF_MEM_KEY][roomName]&&heap[INTEL_EFF_MEM_KEY][roomName].active)return;
    ...
    heap[INTEL_EFF_MEM_KEY][roomName]={active:true,roomName,startTick:Game.time,totals};
...
function _itProcessProfiles() {
    ...
    for(const roomName in heap[INTEL_EFF_MEM_KEY]){
        const profile=heap[INTEL_EFF_MEM_KEY][roomName];
...
function _itFinalise(roomName, profile) {
    ...
    delete heap[INTEL_EFF_MEM_KEY][roomName];
...
function _itIsProfileActive(roomName){return !!(heap[INTEL_EFF_MEM_KEY]&&heap[INTEL_EFF_MEM_KEY][roomName]&&heap[INTEL_EFF_MEM_KEY][roomName].active);}
...
function _itAnalyzeEco(room, effCache, fp) {
    ...
    const roomOrders=util.marketSnapshot().all.filter(o=>o.roomName===room.name);
...
function _itSecEco(L,res,wt){
    L.push('📊 ECONOMIC ['+wt+']: '+res.economic.score+'/100 | VALUE: '+fmtCr(res.economic.value.total)+' (structs: '+fmtCr(res.economic.value.structureCredits)+' | res: '+fmtCr(res.economic.value.resourceCredits)+')');
    ...
    if(res.fast)L.push('   Production rates unavailable — run intel(\''+res.room+'\') for full profile.');
    else L.push('   Production: Labs: '+ed.labUtilRate+' ('+(ed.labReactions||0)+' rxns) | Terminal: '+ed.terminalSendRate+' ('+(ed.terminalSends||0)+' sends) | Factory: '+(ed.factoryProduces||0)+' produces');
}
function _itSecMil(L,res,wt){
    L.push('⚔️  MILITARY ['+wt+']: '+res.military.score+'/100 | VALUE: '+fmtCr(res.military.value.total));
...
function _itSecDP(L,res,wt){
    L.push('🔧 DUAL PURPOSE ['+wt+']: '+res.dualPurpose.score+'/100 | VALUE: '+fmtCr(res.dualPurpose.value.total));
...
function intel(roomName) {
    ...
    if(!effCache&&_itIsProfileActive(roomName)){const p=heap[INTEL_EFF_MEM_KEY][roomName];console.log('[Intel] Profiling '+roomName+': '+(p?p.totals.ticksObserved:0)+'/'+INTEL_PROFILE_TICKS+' ticks. Report auto-prints when complete.');return;}
...
function listIntel() {
    const cache=Memory[INTEL_EFF_CACHE_KEY],active=heap[INTEL_EFF_MEM_KEY];
...
function _enStartProfile(target) {
    ...
    if (heap[INTEL_EFF_MEM_KEY] && heap[INTEL_EFF_MEM_KEY][target] && heap[INTEL_EFF_MEM_KEY][target].active) {
...
function _regRecord(roomName, room) {
    ...
    if (room.controller && room.controller.my) {
        ...
        if (Memory.playerMonitor) {
            if (Memory.playerMonitor.state) delete Memory.playerMonitor.state[roomName];
            if (Memory.playerMonitor.awaiting) delete Memory.playerMonitor.awaiting[roomName];
        }
        return;
    }
...
function _regPruneUnreachable(reg) {
    for (const rn in reg.rooms) if (!findObserverInRange(rn)) delete reg.rooms[rn];
}
...
const MON_BOOLEAN_FIELDS = [
    'safeMode', 'hasNuker', 'nukerHasG', 'nukerHasE', 'nukerReady',
    'hasCreeps', 'hasMilitary', 'hasBoosted', 'spawnsDrained',
    'hasPower', 'terminalHasG', 'towersLow'
];

function _monCompactState(state) {
    if (!state || typeof state !== 'object') return state;
    delete state.owned;
    for (const field of MON_BOOLEAN_FIELDS) if (!state[field]) delete state[field];
    if (state.incomingNukes && Object.keys(state.incomingNukes).length === 0) delete state.incomingNukes;
    return state;
}
...
function _monMem() {
    ...
    if (mem.schemaVersion === undefined || mem.schemaVersion < MON_SCHEMA_VERSION) {
        for (const rn in mem.state) _monCompactState(mem.state[rn]);
        for (const pName in mem.players) {
            const pd = mem.players[pName];
            mem.players[pName] = { status: (pd && pd.status) || STATUS.ENEMY };
        }
        mem.schemaVersion = MON_SCHEMA_VERSION;
        console.log('[Monitor] Compacted player monitor memory to schema v' + MON_SCHEMA_VERSION + '.');
    }
    return mem;
}

function _monPruneState(reg) {
    const mem = Memory.playerMonitor;
    if (!mem || !mem.state) return;
    for (const rn in mem.state) if (!reg.rooms[rn]) delete mem.state[rn];
    if (mem.awaiting)
        for (const rn in mem.awaiting) if (!reg.rooms[rn]) delete mem.awaiting[rn];
}
...
function _monFire(playerName, room, type, message) {
    ...
    if (!pd && !UNMONITORED_ALERTS.includes(type)) return;
    const filter = pd ? ALERT_FILTER[pd.status || STATUS.ENEMY] : UNMONITORED_ALERTS;
 ...
 *
 * State fields:
 *   rcl, safeMode, hasNuker, nukerHasG, nukerHasE, nukerReady,
 *   incomingNukes, hasMilitary,
...
function analyzeRoom(room, playerName, prev, status) {
    ...
    if (s.hasNuker) {
        ...
        s.nukerHasG = gAmt > 0;
        s.nukerHasE = eAmt > 0;
        s.nukerReady = gAmt >= 5000 && eAmt >= 300000;
        ...
        if (!first) {
            ...
            if (!prev.nukerReady && s.nukerReady) {
                _monFire(playerName, roomName, 'NUKER_READY',
                         'Nuker is ready to launch (' + gAmt + '/5000 G, ' + eAmt + '/300000 energy)');
            }
        }
    } else {
        s.nukerHasG = false;
    ...
    // ── Incoming nukes against this player room ──────────────────
    const incomingNukes = room.find(FIND_NUKES);
    s.incomingNukes = {};
    for (const nuke of incomingNukes) {
        s.incomingNukes[nuke.id] = {
            x: nuke.pos.x,
            y: nuke.pos.y,
            launchRoom: nuke.launchRoomName,
            landTick: Game.time + nuke.timeToLand
        };
        if (!prev.incomingNukes || !prev.incomingNukes[nuke.id]) {
            _monFire(playerName, roomName, 'NUKE_INCOMING',
                     'Incoming nuke will land at (' + nuke.pos.x + ',' + nuke.pos.y +
                     ') in ' + nuke.timeToLand + ' ticks. Launched from: ' + nuke.launchRoomName);
        }
    }
    ...
    s.t = Game.time;
    return _monCompactState(s);
}

/** Deep-analyze visible player rooms. Statused players get the full alert set;
 *  unstatused players only emit UNMONITORED_ALERTS and are not hot-polled. */
function _monAnalyzeIfTracked(room, owner) {
    const mem = _monMem();
    const pd = mem.players[owner];
    const prev = mem.state[room.name] || {};
    mem.state[room.name] = analyzeRoom(room, owner, prev, pd ? (pd.status || STATUS.ENEMY) : STATUS.ENEMY);
...
// ════════════════════════════════════════════════════════════════════════════
// ████████████████████████  PLAYER STATUS BOARD  █████████████████████████████
// status('PlayerName') routes here from statusReport.js. Snapshots every
// registry room the player owns through the observer scheduler (one room per
// observer per tick at one-shot priority), then hands the collected data back
// to statusReport.printPlayerStatus() — scanner gathers, statusReport prints.
// ════════════════════════════════════════════════════════════════════════════

const PST_TIMEOUT    = 50;       // give up on rooms still unobserved after this many ticks
const PST_ANCHOR_TTL = 100000;   // drop RCL-ETA anchors for players not scanned in this long

// Name keywords → category, checked in order (first substring hit wins).
// The name only refines the body classification, never overrides it: the
// matched category must belong to the same body family (PST_NAME_BODY),
// so an enemy "harvester" with a hauler body still renders as a hauler,
// and military bodies ignore names entirely.
const PST_NAME_CATS = [
    ['upgrad',    'upgrader'],
    ['build',     'builder'],
    ['repair',    'repairer'],
...
// Body family each name category must match before the name is believed
const PST_NAME_BODY = {
    upgrader:   'worker',
    builder:    'worker',
    repairer:   'worker',
...
function _pstCategorize(creep) {
    const body = creep.body, total = body.length;
    let atk = 0, ra = 0, heal = 0, work = 0, claim = 0, move = 0, boosted = false;
    for (const p of body) {
        if (p.boost) boosted = true;
        if (p.type === ATTACK) atk++;
        else if (p.type === RANGED_ATTACK) ra++;
        else if (p.type === HEAL) heal++;
        else if (p.type === WORK) work++;
        else if (p.type === CLAIM) claim++;
        else if (p.type === MOVE) move++;
    }
    if (atk && atk >= ra && atk >= heal) return { cat: 'melee',  boosted };
    if (ra && ra >= heal)                return { cat: 'ranged', boosted };
    if (heal)                            return { cat: 'healer', boosted };

    let bodyCat;
    if (claim)                    bodyCat = 'claimer';
    else if (work / total > 0.25) bodyCat = 'worker';
    else if (move === total)      bodyCat = 'scout';
    else if (work / total < 0.10) bodyCat = 'hauler';
    else                          bodyCat = 'other';

    const name = creep.name.toLowerCase();
    for (const [kw, cat] of PST_NAME_CATS) {
        if (name.indexOf(kw) === -1) continue;
        if (PST_NAME_BODY[cat] === bodyCat) return { cat, boosted };
        break;
    }
    return { cat: bodyCat, boosted };
}

function _pstCollectRoom(room, playerName) {
    const c = room.controller;
    const owner = (c && c.owner) ? c.owner.username : null;
    if (owner !== playerName) return { name: room.name, ok: true, lost: true, owner: owner, t: Game.time };

    const structs = room.find(FIND_HOSTILE_STRUCTURES);
    const byType = Object.create(null);
    for (const st of structs) (byType[st.structureType] || (byType[st.structureType] = [])).push(st);

    const towers = byType[STRUCTURE_TOWER] || [];
    const spawns = byType[STRUCTURE_SPAWN] || [];
    const nukers = byType[STRUCTURE_NUKER] || [];

    // room.energyAvailable reads 0 for hostile rooms — sum spawn/extension stores
    let enAvail = 0, enCap = 0;
    for (const st of spawns.concat(byType[STRUCTURE_EXTENSION] || [])) {
        if (!st.store) continue;
        enAvail += st.store.getUsedCapacity(RESOURCE_ENERGY) || 0;
        enCap   += st.store.getCapacity(RESOURCE_ENERGY) || 0;
    }

    const counts = {};
    let boosted = 0, totalCreeps = 0;
    for (const creep of room.find(FIND_HOSTILE_CREEPS)) {
        if (creep.owner.username !== playerName) continue;
        const r = _pstCategorize(creep);
        counts[r.cat] = (counts[r.cat] || 0) + 1; totalCreeps++;
        if (r.boosted) boosted++;
    }
    let power = 0;
    for (const pc of room.find(FIND_HOSTILE_POWER_CREEPS))
        if (pc.owner.username === playerName) power++;

    return {
        name: room.name, ok: true, lost: false, owner: owner,
        rcl: c.level,
        pct: (c.level < 8 && c.progressTotal > 0) ? c.progress / c.progressTotal * 100 : null,
        safeMode: c.safeMode || 0,
        enAvail: enAvail, enCap: enCap,
        storageE: room.storage ? (room.storage.store.getUsedCapacity(RESOURCE_ENERGY) || 0) : null,
        terminalE: room.terminal ? (room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) || 0) : null,
        towers: towers.length,
        towersLow: towers.length > 0 && towers.every(t => (t.store.getUsedCapacity(RESOURCE_ENERGY) || 0) < 10),
        spawns: spawns.length,
        spawning: spawns.filter(s => s.spawning).length,
        nuker: nukers.length > 0,
        nukerG: nukers.length ? (nukers[0].store.getUsedCapacity(RESOURCE_GHODIUM) || 0) : 0,
        nukerE: nukers.length ? (nukers[0].store.getUsedCapacity(RESOURCE_ENERGY) || 0) : 0,
        creeps: counts, totalCreeps: totalCreeps, boosted: boosted, power: power,
        t: Game.time,
    };
}

function _pstProgress(state) {
    const done = Object.keys(state.results).length, total = done + Object.keys(state.pending).length;
    console.log('[Status] Scanning ' + state.player + ': ' + done + '/' + total + ' room(s) (' + (Game.time - state.startTick) + 't elapsed).');
}

function startPlayerStatusScan(playerName) {
    if (!playerName || typeof playerName !== 'string') { console.log('[Status] Usage: status("PlayerName")'); return; }
    const active = Memory.playerStatusScan;
    if (active && active.active) {
        if (active.player === playerName) { _pstProgress(active); return; }
        console.log('[Status] A scan for ' + active.player + ' is already running (' + Object.keys(active.pending).length + ' room(s) left).');
        return;
    }
    const rooms = _regRoomsOf(playerName);
    if (!rooms.length) {
        const reg = _regMem();
        console.log('[Status] Registry has no rooms for "' + playerName + '".');
        if (reg.sweep && reg.sweep.active)
            console.log('[Status] Registry sweep in progress (' + reg.sweep.scanned + '/' + reg.sweep.total + ') — retry when it completes.');
        else if (!_regFresh()) {
            _regStartSweep();
            console.log('[Status] Registry stale — sweep started; re-run status("' + playerName + '") when it completes (registryStatus() for progress).');
        } else
            console.log('[Status] Registry is fresh — check the spelling (registryList() shows known players), or seed rooms with monitorAdd().');
        return;
    }

    if (Memory.playerStatusAnchors)
        for (const p in Memory.playerStatusAnchors)
            if (Game.time - Memory.playerStatusAnchors[p].t > PST_ANCHOR_TTL) delete Memory.playerStatusAnchors[p];

    const state = { active: true, player: playerName, startTick: Game.time, pending: {}, results: {} };
    for (const rn of rooms) {
        const room = Game.rooms[rn];
        if (room) { state.results[rn] = _pstCollectRoom(room, playerName); _regRecord(rn, room); }
        else if (obsRequest(rn, 'status', OBS_PRI.ONESHOT)) state.pending[rn] = Game.time;
        else state.results[rn] = { name: rn, ok: false, reason: 'range' };
    }
    Memory.playerStatusScan = state;
    const left = Object.keys(state.pending).length;
    if (!left) { _pstComplete(state); return; }
    console.log('[Status] Scanning ' + playerName + ': ' + rooms.length + ' room(s), ' + left + ' awaiting observers...');
}

function _pstTick() {
    const state = Memory.playerStatusScan;
    if (!state || !state.active) return;
    for (const rn in state.pending) {
        const room = Game.rooms[rn];
        if (room) {
            state.results[rn] = _pstCollectRoom(room, state.player);
            _regRecord(rn, room);
            delete state.pending[rn];
        } else if (Game.time - state.pending[rn] > PST_TIMEOUT) {
            state.results[rn] = { name: rn, ok: false, reason: 'timeout' };
            delete state.pending[rn];
        } else {
            obsRequest(rn, 'status', OBS_PRI.ONESHOT);
        }
    }
    if (!Object.keys(state.pending).length) _pstComplete(state);
}

function _pstComplete(state) {
    if (!Memory.playerStatusAnchors) Memory.playerStatusAnchors = {};
    const prev = Memory.playerStatusAnchors[state.player];
    const nextAnchors = {};
    const roomList = [];
    for (const rn of Object.keys(state.results).sort()) {
        const r = state.results[rn];
        if (r.ok && !r.lost) {
            r.etaTicks = null;
            const a = prev && prev.rooms && prev.rooms[rn];
            if (r.pct !== null && a && a.l === r.rcl && r.t - a.t >= 100 && r.pct > a.pct)
                r.etaTicks = Math.ceil((100 - r.pct) / ((r.pct - a.pct) / (r.t - a.t)));
            if (r.pct !== null) nextAnchors[rn] = { l: r.rcl, pct: r.pct, t: r.t };
        }
        roomList.push(r);
    }
    Memory.playerStatusAnchors[state.player] = { t: Game.time, rooms: nextAnchors };
    const mem = _monMem();
    const reg = _regMem();
    const payload = {
        player: state.player,
        status: mem.players[state.player] ? (mem.players[state.player].status || STATUS.ENEMY) : null,
        elapsed: Game.time - state.startTick,
        registryAge: reg.lastSweepEnd ? Game.time - reg.lastSweepEnd : null,
        rooms: roomList,
    };
    delete Memory.playerStatusScan;
    require('statusReport').printPlayerStatus(payload);
}
