...
// Ticks between full watch-list sweeps (all watched rooms submitted to the
// observer scheduler in one batch).
...
// Periodic deposit sweeps are batch-submitted and not urgent — keep them below
// monitor polls (50) so they soak up leftover observer time instead of competing.
...
function initMemory() {
  ...
  // MIGRATION: watch entries used to be { nextScanTick, lastObserved } objects.
  // The watch list is now just a set of room names; sweep timing is global.
  for (var wRoom in Memory.depositObserver.rooms) {
    if (Memory.depositObserver.rooms[wRoom] !== 1) Memory.depositObserver.rooms[wRoom] = 1;
  }
...
function harvestResources(targetRoomName) {
  ...
  if (!Memory.depositObserver.rooms[targetRoomName]) {
    Memory.depositObserver.rooms[targetRoomName] = 1;
    // Fold the new room into the next sweep immediately.
...
function cancelJobsInRoom(targetRoomName) {
  ...
  var count = 0;
  for (var id in jobs) {
    if (jobs[id].roomName === targetRoomName && !jobs[id].completed) {
      // Mark completed instead of deleting so returning creeps keep their
      // routeBack; maintainJobs reaps the job once its creeps are gone.
...
function buildRoomCostMatrix(roomName) {
  if (!global.__depositObserverCostMatrices || global.__depositObserverCostMatrices.tick !== Game.time) {
    global.__depositObserverCostMatrices = { tick: Game.time, rooms: {} };
  }
  var cached = global.__depositObserverCostMatrices.rooms[roomName];
  if (cached) return cached.clone();

  var room = Game.rooms[roomName];
  var matrix = new PathFinder.CostMatrix();
  if (!room) {
    global.__depositObserverCostMatrices.rooms[roomName] = matrix;
    return matrix.clone();
  }
...
function maintainJobs() {
  ...
  for (var id in Memory.depositObserver.jobs) {
    ...
    // These legacy route derivatives were never consumed and can be rebuilt.
    delete job.routeBack;
    delete job.path;
    delete job.pathBack;
    delete job.pathRooms;
    delete job.exitDirections;
    delete job.exitDirectionsBack;
    ...
    // Completed jobs must outlive their creeps: the job carries the only copy
    // of the route (the return route is derived from it), so
    // deleting early strands returning creeps with ERR_NOT_FOUND.
    if (job.completed) {
      if (job.creeps.length === 0) {
        idsToDelete.push(id);
      } else {
        statusReport.push({ room: job.roomName, status: '🏁 Done, ' + job.creeps.length + ' returning' });
      }
      continue;
    }
...
function spawnHarvesterForJob(job) {
  ...
  var result = spawnManager.spawnCustomCreep(spawn, body, name, mem);
  if (result === OK) {
...
function scanVisibleRoomsForDeposits() {
  var mem      = Memory.depositObserver;
  var roomsMem = mem.rooms;

  // SWEEP: every SCAN_INTERVAL ticks, batch-submit ALL watched rooms lacking
  // vision to the scanner's observer scheduler. The scheduler dedupes, spreads
  // the scans across free observers over the following ticks, and drops any
  // request it can't serve — low priority means these never crowd out route
  // scans or monitor polls. No per-room bookkeeping needed.
  if (Game.time >= mem.nextSweepTick) {
    for (var roomName in roomsMem) {
      if (!Game.rooms[roomName]) {
        scanner.observe.request(roomName, 'deposit', PRI_DEPOSIT_WATCH);
      }
    }
    mem.nextSweepTick = Game.time + SCAN_INTERVAL;
  }
  ...
  for (var roomName in roomsMem) {
    ...
    for (var k = 0; k < currentDeposits.length; k++) {
      ...
      if ((d.lastCooldown || 0) >= MAX_COOLDOWN) {
        var staleJob = Memory.depositObserver.jobs[d.id];
        if (staleJob && !staleJob.completed) {
          console.log('[DepositObserver] Deposit ' + d.id + ' cooldown too high (' + d.lastCooldown + '). Completing job.');
          staleJob.completed = true;
        }
      }
    }
    ...
    for (var jobId in jobsToCheck) {
      if (jobsToCheck[jobId].roomName === roomName) {
        if (foundDepositIds.indexOf(jobId) === -1 && !jobsToCheck[jobId].completed) {
          console.log('[DepositObserver] Deposit ' + jobId + ' in ' + roomName + ' decayed. Completing job.');
          jobsToCheck[jobId].completed = true;
        }
      }
    }
