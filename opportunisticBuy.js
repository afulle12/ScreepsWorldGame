...
function process() {
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        function attemptDeal(order) {
            ...
            if (candidate > maxAffordableByCredits) candidate = maxAffordableByCredits;
            ...
            if (result === OK) {
                ...
                console.log('[OpportunisticBuy] Placed deal for ' + candidate + ' ' + request.resourceType + ' from ' + order.roomName +
        ...
        if (!attempted) {
            ...
            // Among the cheapest price, pick the one allowing the largest feasible buy this tick
            var minPrice = orders[0].price;
            var bestOrder = null;
            var bestFeasible = 0;
            ...
            if (bestOrder && bestFeasible > 0) {
                ...
                // Fallback: first feasible across all filtered orders
                for (var k = 0; k < orders.length && !attempted; k++) {
                    var ord2 = orders[k];
