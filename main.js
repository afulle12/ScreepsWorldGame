...
// screeps-profiler wraps every registered function with per-call overhead,
// every tick, whether or not a profile is actively being captured. Keep it
// opt-in via Memory so normal play doesn't pay that tax. Toggle with the
// profilerOn()/profilerOff() console commands (registered below); takes
// effect on the next global reset.
const PROFILER_ACTIVE = !!(Memory.settings && Memory.settings.screepsProfiler);
if (PROFILER_ACTIVE) profiler.enable();
...
// Serialization happens only on memoryManager save ticks (1 in 10) and
// costs ~1.5 CPU at current Memory size (measured via memoryProfile(),
// 2026-07-06). calcTickBudget() caps the budget on save ticks so they
// don't overrun 20; all other ticks pay no serialization at all.
...
const CPU_LIMITS = {
  HARD_CEILING: 19.3,   // Never exceed (non-save ticks)
  NORMAL:       18.0,   // Comfortable cruising altitude
  CRITICAL:     14.0,   // Survival mode - economy + defense only
};
// Measured JSON.stringify(Memory) cost plus margin, reserved on save ticks.
const MEMORY_SAVE_RESERVE = 1.7;
...
global.profilerOn = function() {
  if (!Memory.settings) Memory.settings = {};
  Memory.settings.screepsProfiler = true;
  return 'Profiler armed. Takes effect on next global reset (push any code change to force one).';
};
global.profilerOff = function() {
  if (!Memory.settings) Memory.settings = {};
  Memory.settings.screepsProfiler = false;
  return 'Profiler disarmed. Takes effect on next global reset (push any code change to force one).';
};
...
function calcTickBudget() {
  ...
  if (Game.time % memoryManager.SAVE_INTERVAL === 0) {
    tickBudget = Math.min(tickBudget, 20 - MEMORY_SAVE_RESERVE);
  }
...
/**
 * Role-specific cleanup for a creep that just died. Called from the
 * every-tick prune loop in runCreeps() — must run immediately, not on a
 * 1000-tick cadence, since Memory.creeps[name] is deleted the same tick
 * the creep disappears from Game.creeps.
 */
function onCreepDeath(name, creepMemory) {
  ...
  // DD_Work workers dying with meaningful TTL left were killed, not expired.
  // lastTTL/lastRoom are recorded every tick by roleTowerDrain.run().
  if (creepMemory.role === 'towerDrain' && creepMemory.variant === 'drainDemolisher' &&
      typeof creepMemory.lastTTL === 'number' && creepMemory.lastTTL > 9) {
    const msg = '[DrainDemolisher] Worker ' + name + ' died with ' + creepMemory.lastTTL +
      ' TTL in ' + (creepMemory.lastRoom || 'unknown room') +
      ' (target ' + creepMemory.targetRoom + ')';
    console.log(msg);
    Game.notify(msg);
  }
...
/**
 * Periodic Memory housekeeping unrelated to per-creep cleanup (which now
 * happens every tick in runCreeps()).
 */
function cleanMemory() {
  ...
  // Orphaned keys (no readers) and telemetry that moved to memoryManager.heap.
  delete Memory.lastEnergyProfile;
  delete Memory.scoutIntel;
  delete Memory.cpuStats;
  delete Memory.cpuProfile;
  delete Memory.cpuProfileLastUsed;
  delete Memory.cpuProfileCreeps;
  delete Memory.roomEffProfile;
}

function trackCPUUsage() {
  if (!heap.cpuStats) {
    heap.cpuStats = { history: [], average: 0, snapshots: [] };
  }
  const cpuUsed = Game.cpu.getUsed();
  heap.cpuStats.history.push(cpuUsed);
  if (heap.cpuStats.history.length > 50) heap.cpuStats.history.shift();
  heap.cpuStats.average = heap.cpuStats.history.reduce(function(sum, cpu) { return sum + cpu; }, 0) / heap.cpuStats.history.length;

  heap.cpuStats.lastBudgetTier = tickBudgetTier;
  heap.cpuStats.lastBudget = tickBudget;
  heap.cpuStats.lastPressure = Math.round(tickPressure * 100);
...
function drawCpuHud() {
  ...
  v.text('Avg: ' + (heap.cpuStats && heap.cpuStats.average ? heap.cpuStats.average.toFixed(1) : '?'),
  ...
  const suspRooms = roomSuspender.getSuspendedRooms();
  v.text('Suspended:', 0.3, 4.85, { color: '#aaaaaa', font: '0.4 monospace', align: 'left' });
  v.text(suspRooms.length ? suspRooms.join(',') : 'none',
    3.8, 4.85, { color: suspRooms.length > 0 ? '#ffaa00' : '#44ff44', font: '0.4 monospace', align: 'left' });

  if (heap.cpuStats && heap.cpuStats.history && heap.cpuStats.history.length > 1) {
    var hist = heap.cpuStats.history.slice(-20);
    var maxH = 20;
  ...
  for (const key in tickSectionCpu) {
    if (key === 'runCreeps') {
      ...
      measuredTotal += tickSectionCpu[key];
  ...
  const untrackedCpu = Math.max(0, used - measuredTotal);
  if (untrackedCpu >= 0.05) {
    moduleEntries.push({ name: 'untracked/overhead', cpu: untrackedCpu });
  }
...
function profileSection(name, fn) {
  ...
  try {
    ...
    if (ENABLE_CPU_LOGGING) {
      if (!heap.cpuProfile[name]) heap.cpuProfile[name] = [];
      heap.cpuProfile[name].push(used);
      if (heap.cpuProfile[name].length > 50) heap.cpuProfile[name].shift();
      if (!heap.cpuProfileLastUsed) heap.cpuProfileLastUsed = {};
      heap.cpuProfileLastUsed[name] = Game.time;
    }
  }
}
...
function cleanCpuProfileMemory(maxAge) {
  ...
  const now = Game.time;
  for (const key in heap.cpuProfileLastUsed) {
    if (now - heap.cpuProfileLastUsed[key] > maxAge) {
      delete heap.cpuProfile[key];
      delete heap.cpuProfileLastUsed[key];
    }
  }
}
...
function runCreeps() {
  ...
  for (const name in Memory.creeps) {
    if (!Game.creeps[name]) {
      try {
        onCreepDeath(name, Memory.creeps[name]);
      } catch (e) {
        handleError('onCreepDeath.' + name, e);
      }
  ...
  for (let i = 0; i < creeps.length; i++) {
    ...
    if (roomSuspender.shouldIdleCreep(creep)) {
      creepsSuspended++;
      continue;
    }
    ...
    // Suspended harvesters get a renewal fast path that must run every tick
    // to keep TTL alive, so bypass the normal throttle for them.
    const suspendedHarvester = roomSuspender.canRunSuspendedHarvester(creep);

    // Throttle gate: compute interval based on priority + live pressure
    const interval = suspendedHarvester ? 1 : getCreepInterval(priority);
    if (interval > 1) {
    ...
    try {
      switch (role) {
        ...
        case 'towerDrain':          roleTowerDrain.runCreep(creep);         break;
        ...
        case 'drainDemolisher':     roleDrainDemolisher.run(creep);         break;
    ...
    if (ENABLE_CPU_LOGGING) {
      ...
      if (!heap.cpuProfileCreeps[role]) heap.cpuProfileCreeps[role] = [];
      heap.cpuProfileCreeps[role].push(cpuUsed);
      if (heap.cpuProfileCreeps[role].length > 50) {
        heap.cpuProfileCreeps[role].shift();
      }
    }
  }
  ...
  if (Memory.operators) {
    for (const name in Memory.operators) {
      ...
      if (!pc) continue;
      if (roomSuspender.isSuspended(Memory.operators[name].homeRoom)) {
        creepsSuspended++;
        continue;
      }
      if (!pc.ticksToLive) {
      ...
      if (ENABLE_CPU_LOGGING) {
        ...
        heap.cpuProfileCreeps['operator'].push(pcCpuUsed);
  ...
  if (Game.time % 50 === 0 && ENABLE_CPU_LOGGING && !DISABLE_CPU_CONSOLE) {
    for (const role in heap.cpuProfileCreeps) {
      const arr = heap.cpuProfileCreeps[role];
...
function tick() {
  ...
  const tickBudgetStart = Game.cpu.getUsed();
  calcTickBudget();
  tickSectionCpu['calcTickBudget'] = Game.cpu.getUsed() - tickBudgetStart;
  ...
  measureSection('creepProfiler.updateGlobalCreepCaches', function() {
    creepProfiler.updateGlobalCreepCaches();
  });
  ...
  measureSection('roomSuspender.run', function() { roomSuspender.run(); });
  profileSection('roomCPUProfiler', function() { roomCPUProfiler.run(); });

  let perRoomRoleCounts = null;
  if (Game.time % 10 === 0 || Game.time % 100 === 0) {
    perRoomRoleCounts = measureSection('statusReport.getPerRoomRoleCounts', function() {
  ...
  if (Game.time % 10 === 0) profileSection('repairManager.run', function() { repairManager.run(); });
  ...
  profileSection('marketBuyer.run', function() { marketBuyer.run(); });
  ...
  measureSection('scanner.run', function() { scanner.run(); });

  if (Game.time % 10 === 0) profileSection('opportunisticSell', function() { opportunisticSell.process(); });
  if (Game.time % 1000 === 0) measureSection('storageManager.cleanStale', function() { storageManager.cleanStale(20000); });
  if (Game.time % 50 === 0) profileSection('marketSeller.run', function() { marketSeller.run(); });

  profileSection('powerManager', function() {
    const ownedRoomNames = getRoomState.ownedNames();
    for (let i = 0; i < ownedRoomNames.length; i++) {
      powerManager.run(ownedRoomNames[i]);
    }
  });
  ...
  profileSection('labManager', function() {
    const ownedRoomNames = getRoomState.ownedNames();
    for (let i = 0; i < ownedRoomNames.length; i++) {
      labManager.run(ownedRoomNames[i]);
    }
  });
  ...
  measureSection('taskScheduler.run', function() { taskScheduler.run(); });
  // --- STATUS DISPLAY ---
  if (Game.time % 100 === 0) {
    ...
    if (ENABLE_CPU_LOGGING && !DISABLE_CPU_CONSOLE) {
      measureSection('cpuProfileConsole', function() {
        for (const key in heap.cpuProfile) {
          const avg = heap.cpuProfile[key].reduce(function(a, b) { return a + b; }, 0) / heap.cpuProfile[key].length;
  ...
  measureSection('cpuThrottleLog', function() {
    if ((Game.time % 10 === 0) && (sectionsThrottled > 0 || creepsThrottled > 0 || creepsSuspended > 0)) {
      const suspRooms = roomSuspender.getSuspendedRooms();
      console.log('[CPU] Throttled: ' + sectionsThrottled + ' sections, '
        + creepsThrottled + ' creeps, '
        + creepsSuspended + ' suspended (rooms: ' + (suspRooms.length ? suspRooms.join(',') : 'none') + ') | Pressure: '
...
module.exports.loop = function() {
  try {
    // Must run before anything touches Memory, and never throttled.
    memoryManager.run();
    if (PROFILER_ACTIVE) profiler.wrap(tick);
    else tick();
  } catch (e) {
