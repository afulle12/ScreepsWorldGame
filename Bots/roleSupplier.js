...
function factoryFetchKey(a, resourceType) {
    if (!a || !resourceType) return null;
    return a.type + '|' + a.taskId + '|' + (a.targetId || '') + '|' + resourceType;
}

function markFactoryFetched(creep, a, resourceType) {
    var key = factoryFetchKey(a, resourceType);
    if (key) creep.memory.ff = key;
}

function hasFactoryFetched(creep, a, resourceType) {
    var key = factoryFetchKey(a, resourceType);
    return !!(key && creep.memory.ff === key);
}
...
function recoverFactoryInputAssignment(creep, view) {
    ...
    // Only re-attach cargo while the order is actually taking inputs.
    // Without this, suppliers carrying leftover energy get hijacked into
    // feeding the factory during processing/unloading — the drain task
    // pulls it right back out and the factory overfills with energy.
    if ((activeOrder.phase || "loading") !== "loading") return false;

    var carriedRes = getCarriedFactoryInputResource(creep, recipe);
    if (!carriedRes) return false;
    if (factoryInputDeficit(view.factory, activeOrder, recipe, carriedRes) <= 0) return false;
...
// Input targets are for the batches NOT yet produced. A cycle that bounced
// back to loading mid-production already consumed inputs for the queued
// batches — targeting the full cycle would reload inputs for batches that
// already ran (overfilling the factory, wasting inputs on completed orders).
function factoryCycleRemainingBatches(order) {
    if (!order) return 0;
...
function factoryCycleTargetTotal(order, recipe) {
    ...
    var batches = factoryCycleRemainingBatches(order);
    var total = 0;
...
function factoryInputDeficit(factory, order, recipe, resourceType) {
    ...
    var need = (recipe.inputs[resourceType] || 0) * factoryCycleRemainingBatches(order);
...
function scanRoomTasks(room, view) {
    ...
    if (view.factory) {
        ...
        if (activeOrder) {
            ...
            if (phase === "loading" && recipe && recipe.inputs) {
                var cycleBatches = factoryCycleRemainingBatches(activeOrder);
...
function pickTask(creep, view, h) {
    ...
    for (var i = 0; i < tasks.length; i++) {
        ...
        } else if (t.type === "factory_input" || t.type === "factory_output" || t.type === "factory_drain") {
            var fRes = parseExtra(t.extra || '', "res");
            creep.memory.s = (fRes && (creep.store[fRes] || 0) > 0) ? "delivering" : "fetching";
...
function isAssignmentDone(creep, view, a, h) {
    ...
    if (a.type === "factory_input") {
        ...
        var needIn = (recipeIn.inputs[factoryRes] || 0) * factoryCycleRemainingBatches(orderIn);
    ...
    if (a.type === "factory_drain") {
        ...
        // Drain tasks are valid WITHOUT an active order (idle-factory
        // cleanup) — requiring one here cancelled every no-order drain one
        // tick after pickup, leaving full factories untouched.
        ...
        if ((creep.store[drainRes] || 0) > 0) return false;
...
function simulateFillDone(a, view, target, sent) {
    ...
    if (a.type === "factory_input") {
        ...
        var needIn = (recipeIn.inputs[inputRes] || 0) * factoryCycleRemainingBatches(orderIn);
...
function dumpAll(creep, room) {
    ...
    for (var i = 0; i < keys.length; i++) {
        var res = keys[i];
        if ((creep.store[res] || 0) <= 0) continue;
        return dumpResource(creep, room, res);
    }
    return false;
}
...
function cleanupCargoForTask(creep, a) {
    ...
    if (a && (a.type === 'factory_output' || a.type === 'factory_drain')) {
        var factoryOutRes = getFactoryTaskResource(a);
        if (creep.store.getUsedCapacity() <= 0) return false;
        if (factoryOutRes && onlyCarriesResource(creep, factoryOutRes) && hasFactoryFetched(creep, a, factoryOutRes)) return false;
        if (factoryOutRes && (creep.store[factoryOutRes] || 0) > 0) {
            if (dumpResource(creep, creep.room, factoryOutRes)) return true;
        }
        if (dumpAll(creep, creep.room)) return true;
        clearAssignment(creep, "factory cargo blocked", true);
        return true;
    }
...
function executeFetch(creep, a, view, h) {
    ...
    if (a.type === "factory_output" || a.type === "factory_drain") {
        ...
        if (a.type === "factory_drain") {
            outAvailable = Math.max(0, outAvailable - (+parseExtra(a.extra, "keep") || 0));
        }
        if (outAvailable <= 0) {
            clearAssignment(creep, "factory drained to keep", true);
            return;
        }
...
function executeDeliver(creep, a, view, h) {
    ...
    if (a.type === "lab_unload") {
        ...
        if (unloadTransfer === OK) {
            refreshProgress(creep, a);
            var unloadOrder = getLabOrder(view.roomName);
            if (unloadOrder && unloadOrder.type === 'production' && unloadRes === unloadOrder.product &&
                labManager && typeof labManager.recordDelivery === 'function') {
                labManager.recordDelivery(view.roomName, unloadRes, unloadSent);
            }
            if (isAssignmentDone(creep, view, a, h)) {
    ...
    if (a.type === "factory_input" || a.type === "factory_output" || a.type === "factory_drain") {
        ...
        if (a.type === "factory_input") {
            ...
            // factoryInputDeficit ignores the phase, and during processing the
            // deficit reappears as inputs are consumed — never top up outside
            // the loading phase or the factory fills with excess energy.
            if (!inputOrder || (inputOrder.phase || "loading") !== "loading") {
                clearAssignment(creep, "factory not loading", true);
                return;
            }
            var inputRecipe = factoryManager.getRecipe(inputOrder.product);
        ...
        if ((a.type === "factory_output" || a.type === "factory_drain") && !hasFactoryFetched(creep, a, factoryRes)) {
            if (dumpResource(creep, creep.room, factoryRes)) return;
            clearAssignment(creep, "factory output cargo not fetched", true);
            return;
        }
        ...
        if (outputTransfer === OK) {
            refreshProgress(creep, a);
            var outputDone = isAssignmentDone(creep, view, a, h);
            if (outputDone) { clearAssignment(creep, "task complete", true); return; }
            if (outputCarried - outputSent <= 0) {
                delete creep.memory.ff;
                creep.memory.s = "fetching";
            }
...
var roleSupplier = {
    run: function(creep) {
        ...
        // If a factory_input load is still genuinely needed (loading phase,
        // deficit open), isAssignmentDone returns false and the trip
        // continues; recoverFactoryInputAssignment re-attaches carried input
        // cargo after transient clears. Forcing "delivering" here regardless
        // of phase was what stuffed factories with excess energy.
