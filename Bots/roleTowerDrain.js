...
//   orderTowerDrain('E1S1', 'E2S1', 1, 'N', '10t5w25m10h') // Custom ordered body: 10 TOUGH, 5 WORK, 25 MOVE, 10 HEAL
//   orderTowerDrain('E1S1', 'E2S1', 2, 'N', null, 'work')         // Standard body, replace ATTACK with WORK (dismantle)
//   orderTowerDrain('E1S1', 'E2S1', 2, 'N', null, 'rangedAttack') // Standard body, replace ATTACK with RANGED_ATTACK
//   orderTowerDrain('E1S1', 'E2S1', 2, 'N', 'long', 'work')         // Long-range body, replace ATTACK with WORK
//   orderTowerDrain('E1S1', 'E2S1', 2, 'N', 'long', 'rangedAttack') // Long-range body, replace ATTACK with RANGED_ATTACK
//   orderBulldozer('E1S1', 'E2S1', 1, 'N')   // 20W/20M/1RA/9H towerdrain variant using 4-position rest stops
//   testBulldozer('E1S1', 'E2S1', 1, 'N')    // Dry run bulldozer planning
//   orderDrainDemolisher('E1S1', 'E2S1', 3, null, {x:2, y:25}) // Up to 3 paired 25W/25M workers + parked 25H/25M healers
//                                             // dismantling the near-edge wall/rampart at (2,25); edge derived from target
//   orderDrainDemolisher('E1S1', 'E2S1', 1, 'N')               // Untargeted: generic drain lanes, dismantle whatever is adjacent
//   testDrainDemolisher('E1S1', 'E2S1', 3, null, {x:2, y:25})  // Dry run drain-demolisher planning
//   updateBulldozerBodies()                   // Update existing bulldozer ops to the current bulldozer body
//   updateDrainDemolisherBodies()             // Update existing drain-demolisher ops to the current worker body
//   setTowerDrainCount('E1S1', 'E2S1', 3)     // Set total units for an existing towerdrain/bulldozer operation
//   addTowerDrainUnits('E1S1', 'E2S1', 1)     // Add units to an existing towerdrain or bulldozer operation
//   addBulldozerUnits('E1S1', 'E2S1', 1)      // Add units to an existing bulldozer operation only
...
//   testTowerDrain('E1S1', 'E2S1', 2, 'N', null, 'work')         // Dry run: standard, replace ATTACK with WORK
//   testTowerDrain('E1S1', 'E2S1', 2, 'N', null, 'rangedAttack') // Dry run: standard, replace ATTACK with RANGED_ATTACK
//   testTowerDrain('E1S1', 'E2S1', 2, 'N', 'long', 'work')         // Dry run: long-range, replace ATTACK with WORK
//   testTowerDrain('E1S1', 'E2S1', 2, 'N', 'long', 'rangedAttack') // Dry run: long-range, replace ATTACK with RANGED_ATTACK
...
// =============================================================================
// 6th param extraPart options:
//   'work'         - replace the ATTACK part with 1 WORK (creep will use dismantle() on structures in range 1)
//   'rangedAttack' - replace the ATTACK part with 1 RANGED_ATTACK (creep will use rangedAttack() at range 3)
//   falsy/omitted  - keep the default ATTACK part
// 5th param body options:
//   'long'          - long-range body (9T/1A/25M/15H)
//   custom spec     - exact ordered body using count + part alias, e.g. '10t5w25m10h'
//   aliases         - t=TOUGH, w=WORK, m=MOVE, c=CARRY, a=ATTACK, r=RANGED_ATTACK, h=HEAL, cl=CLAIM
//   Custom bodies preserve written order, so front-loaded TOUGH parts stay in front.
// =============================================================================
...
var BULLDOZER_BODY_SPEC = '20w20m1r9h';
var DRAIN_DEMOLISHER_BODY_SPEC = '25w25m';
var DRAIN_DEMOLISHER_MAX_PAIRS = 3;
var TOWER_DRAIN_RETREAT_DEBUG = false;
...
var getOppositeEdge = util.getOppositeEdge;
...
function computeLanes(op) {
    ...
    // Targeted mode (drain demolisher): pack lanes onto the depth-1 tiles
    // Chebyshev-adjacent to the wall/rampart segment so every worker can
    // dismantle it from its drain position. No lane spacing.
    if (op.targetPos) {
        var t = (edge === 'E' || edge === 'W') ? op.targetPos.y : op.targetPos.x;
        var wanted = [t, t - 1, t + 1];
        var targeted = [];
        for (var tw = 0; tw < wanted.length; tw++) {
            var wc = wanted[tw];
            if (bothWalkable.indexOf(wc) === -1) continue;
            var probe = buildPositions(edge, wc);
            // The segment can sit on the drain line itself - can't stand there
            if (probe.drainPos.x === op.targetPos.x && probe.drainPos.y === op.targetPos.y) continue;
            targeted.push(wc);
        }

        if (targeted.length === 0) {
            console.log('[TowerDrain] ERROR: No walkable drain tiles adjacent to target (' +
                        op.targetPos.x + ',' + op.targetPos.y + ') on edge ' + edge);
            return false;
        }

        op.lanes = {};
        var targetedLane = 1;
        for (var tj = 0; tj < targeted.length && targetedLane <= count; tj++) {
            var targetedPositions = buildPositions(edge, targeted[tj]);
            targetedPositions.targetPos = op.targetPos;
            op.lanes[String(targetedLane)] = targetedPositions;
            console.log('[TowerDrain] Targeted lane ' + targetedLane + ' at coord ' + targeted[tj] +
                        ': drain=' + JSON.stringify(targetedPositions.drainPos) +
                        ', heal=' + JSON.stringify(targetedPositions.healPos));
            targetedLane++;
        }

        var targetedCount = Object.keys(op.lanes).length;
        if (targetedCount < count) {
            console.log('[TowerDrain] WARNING: Only ' + targetedCount + ' targeted lanes fit around (' +
                        op.targetPos.x + ',' + op.targetPos.y + ') (requested ' + count + ')');
        }
...
function parseTowerDrainBodySpec(spec) {
    if (typeof spec !== 'string') return null;

    var normalized = spec.toLowerCase().replace(/\s+/g, '');
    ...
    while ((match = tokenRe.exec(normalized)) !== null) {
        ...
        var part = aliases[match[2]];
        if (!part || count <= 0) return null;

        for (var i = 0; i < count; i++) body.push(part);
        if (body.length > 50) return null;
    }

    if (consumed !== normalized || body.length === 0) return null;
    return body;
}

function getTowerDrainBodyLabel(op) {
    if (op && op.variant === 'bulldozer') return 'bulldozer (20W/20M/1RA/9H)';
    if (op && op.variant === 'drainDemolisher') return 'drain demolisher (25W/25M + paired 25H/25M healer)';
    if (op && op.bodySpec) return 'custom (' + op.bodySpec + ')';
    return (op && op.longRange ? 'long-range (9T/1A/25M/15H)' : 'standard') +
        (op && op.extraPart ? ' (ATTACK->' + op.extraPart + ')' : '');
}

function getBulldozerOptions(dryRun) {
    return {
        dryRun: !!dryRun,
        longRange: false,
        extraPart: null,
...
function getDrainDemolisherOptions(dryRun, targetPos) {
    return {
        dryRun: !!dryRun,
        longRange: false,
        extraPart: null,
...
function orderTowerDrain(homeRoom, targetRoom, count, preferredEdge, range, extraPart) {
    var options = { longRange: range === 'long' };
    if (range && range !== 'long' && range !== 'standard') {
        options.body = parseTowerDrainBodySpec(range);
        if (!options.body) {
            console.log('[TowerDrain] Invalid body option: ' + range + '. Use "long", "standard", or a custom spec like "10t5w25m10h".');
            return ERR_INVALID_ARGS;
        }
        options.bodySpec = range.toLowerCase().replace(/\s+/g, '');
    }
    if (extraPart !== undefined && extraPart !== null && extraPart !== '' && extraPart !== 'work' && extraPart !== 'rangedAttack') {
        console.log('[TowerDrain] Invalid extraPart: ' + extraPart + '. Use "work" or "rangedAttack".');
        return ERR_INVALID_ARGS;
    }
    if (options.body && extraPart) {
        console.log('[TowerDrain] Cannot combine custom body spec with extraPart. Put all desired parts in the body spec.');
        return ERR_INVALID_ARGS;
    }
    options.extraPart = extraPart || null;
    initMemory();
    ...
    console.log('[TowerDrain] Body type: ' + getTowerDrainBodyLabel(options));
...
function orderTowerDrainSameSector(homeRoom, targetRoom, count, preferredEdge, requiredSafeRoom, observerInfo, opKey, options) {
    ...
    Memory.towerDrainOps.operations[opKey] = {
        ...
        extraPart: options.extraPart || null,
        body: options.body || null,
        bodySpec: options.bodySpec || null,
        variant: options.variant || null,
        targetPos: options.targetPos || null,
        crossSector: false,
        scanData: {
    ...
    if (options.extraPart) {
        console.log('[TowerDrain] Replaced ATTACK with: ' + options.extraPart);
    }
    if (options.variant === 'bulldozer') {
        console.log('[TowerDrain] Variant: bulldozer (20W/20M/1RA/9H)');
    } else if (options.bodySpec) {
        console.log('[TowerDrain] Custom body: ' + options.bodySpec);
    }
...
function setTowerDrainCount(homeRoom, targetRoom, count) {
    initMemory();

    count = parseInt(count, 10);
    if (!count || count < 1) {
        console.log('[TowerDrain] Invalid count: ' + count + '. Use a positive integer.');
        return ERR_INVALID_ARGS;
    }
    ...
    if (op.status !== 'ready' && op.status !== 'active') {
        console.log('[TowerDrain] Operation ' + opKey + ' is not ready/active (status: ' + op.status + ')');
        return ERR_BUSY;
    }

    var oldCount = op.maxDrainers || 0;
    var oldLaneCount = Object.keys(op.lanes || {}).length;
    var oldLanes = op.lanes || {};
    op.maxDrainers = count;

    if (oldLaneCount < count) {
        if (!computeLanes(op)) {
            op.maxDrainers = oldCount;
            op.lanes = oldLanes;
            console.log('[TowerDrain] Could not compute enough lanes for ' + opKey + '; count restored to ' + oldCount);
            return ERR_NO_PATH;
        }
    }

    var newLaneCount = Object.keys(op.lanes || {}).length;
    console.log('[TowerDrain] Updated ' + opKey + ' count ' + oldCount + ' -> ' + count +
        ' | lanes: ' + newLaneCount + '/' + count +
        ' | body: ' + getTowerDrainBodyLabel(op));

    if (newLaneCount < count) {
        console.log('[TowerDrain] WARNING: Only ' + newLaneCount + ' lanes are available; spawning is limited by lane count.');
    }

    return OK;
}

function addTowerDrainUnits(homeRoom, targetRoom, amount) {
    initMemory();
    ...
    return setTowerDrainCount(homeRoom, targetRoom, (op.maxDrainers || 0) + amount);
}

function addBulldozerUnits(homeRoom, targetRoom, amount) {
    initMemory();
    ...
    if (op.variant !== 'bulldozer') {
        console.log('[TowerDrain] Operation ' + opKey + ' is not a bulldozer operation. Use addTowerDrainUnits for generic towerdrain operations.');
        return ERR_INVALID_TARGET;
    }

    return addTowerDrainUnits(homeRoom, targetRoom, amount);
}

// Body specs used by earlier revisions of each variant; ops created back then
// may lack the variant field, so match them by spec and stamp the variant on.
var LEGACY_VARIANT_SPECS = {
    bulldozer: ['15t5w20m1r9h'],
    drainDemolisher: ['5t20w25m']
};

function updateVariantBodies(variant, spec) {
    initMemory();

    var ops = Memory.towerDrainOps.operations;
    var body = parseTowerDrainBodySpec(spec);
    var legacySpecs = LEGACY_VARIANT_SPECS[variant] || [];
    var updated = 0;
    ...
    for (var opKey in ops) {
        ...
        var matches = op.variant === variant ||
            (op.bodySpec && legacySpecs.indexOf(String(op.bodySpec).toLowerCase()) !== -1);
        if (!matches) continue;
        ...
        console.log('[TowerDrain] Updated ' + variant + ' body for ' + opKey + ' to ' + spec + ' (' + getTowerDrainBodyLabel(op) + ')');
    }

    if (updated === 0) {
        console.log('[TowerDrain] No ' + variant + ' operations found to update.');
        for (var k in ops) {
            var o = ops[k];
            if (!o) continue;
            console.log('[TowerDrain]   existing op ' + k + ': variant=' + (o.variant || 'none') +
                ', bodySpec=' + (o.bodySpec || 'none') + ', status=' + (o.status || 'unknown'));
        }
    } else {
        console.log('[TowerDrain] Updated ' + updated + ' ' + variant + ' operation(s). Existing creeps keep their current body; future spawns use the new body.');
    }
...
function getTowerDrainStatus() {
    ...
    for (var opKey in ops) {
        ...
        console.log('  Body: ' + getTowerDrainBodyLabel(op));
        ...
        if (op.targetPos) {
            console.log('  Target: (' + op.targetPos.x + ',' + op.targetPos.y + ') in ' + op.targetRoom);
        }
    ...
    while (validPaths.length > 0) {
        ...
        if (routeVerified) {
            ...
            if (op.dryRun) {
                ...
                console.log('[TowerDrain] Body: ' + getTowerDrainBodyLabel(op));
...
function hasLostBodyPart(creep) {
    for (var i = 0; i < creep.body.length; i++) {
        if (creep.body[i].hits <= 0) return true;
    }
    return false;
}

function countLostBodyParts(creep) {
    var lost = 0;
    for (var i = 0; i < creep.body.length; i++) {
        if (creep.body[i].hits <= 0) lost++;
    }
    return lost;
}

function firstActiveBodyPartHits(creep) {
    for (var i = 0; i < creep.body.length; i++) {
        if (creep.body[i].hits > 0) return creep.body[i].hits;
    }
    return 0;
}

// Hostile creep heavy enough to shred a drain-demolisher worker before its
// paired healer can matter: 25+ ATTACK parts in melee range, or 25+
// RANGED_ATTACK parts within firing range (3).
var HEAVY_ATTACKER_PARTS = 25;

function usesLostPartRetreat(creep) {
    return creep.memory.variant === 'drainDemolisher' || creep.memory.variant === 'bulldozer';
}

function hasNearbyHeavyAttacker(creep) {
    var hostiles = creep.pos.findInRange(FIND_HOSTILE_CREEPS, 3, {
        filter: function(hostile) { return iff.isHostileCreep(hostile); }
    });
    for (var i = 0; i < hostiles.length; i++) {
        var attackParts = hostiles[i].getActiveBodyparts(ATTACK);
        var rangedParts = hostiles[i].getActiveBodyparts(RANGED_ATTACK);
        if (rangedParts >= HEAVY_ATTACKER_PARTS) return true;
        if (attackParts >= HEAVY_ATTACKER_PARTS && creep.pos.isNearTo(hostiles[i])) return true;
    }
    return false;
}

function getIncomingPartLossThreat(creep) {
    var exposedHits = firstActiveBodyPartHits(creep);
    if (exposedHits <= 0) return null;
    ...
    for (var i = 0; i < hostiles.length; i++) {
        ...
        var attackParts = hostile.getActiveBodyparts(ATTACK);
        var rangedParts = hostile.getActiveBodyparts(RANGED_ATTACK);

        if (range <= 1 && attackParts * 30 >= exposedHits) {
            return 'incoming part loss from ' + attackParts + ' attack parts at range ' + range;
        }
        if (range <= 3 && rangedParts * 10 >= exposedHits) {
            return 'incoming part loss from ' + rangedParts + ' ranged parts at range ' + range;
        }
    }
    return null;
}

// Live healer partner for a drain-demolisher worker (paired by squadId).
function findSquadHealer(creep) {
    ...
    return _.find(getRoomState.creepIndex().all, function(c) {
        return c && c.memory &&
               c.memory.role === 'drainDemolisher' &&
               c.memory.squadId === squadId;
    });
}

function hasParkedDrainDemolisherHealer(creep) {
    return !!_.find(getRoomState.creepIndex().all, function(c) {
        return c && c.memory &&
               c.memory.role === 'drainDemolisher' &&
               c.memory.homeRoom === creep.memory.homeRoom &&
               c.memory.targetRoom === creep.memory.targetRoom &&
               c.memory.safeRoom === creep.memory.safeRoom &&
               c.memory.state === 'parked' &&
               c.room.name === creep.memory.safeRoom;
    });
}

function getTowerDrainOpKey(creep) {
    if (creep.memory.opKey) return creep.memory.opKey;
    if (creep.memory.homeRoom && creep.memory.targetRoom) {
        creep.memory.opKey = creep.memory.homeRoom + '->' + creep.memory.targetRoom;
        return creep.memory.opKey;
    }
    return null;
}

function getTowerDrainOp(creep) {
    var opKey = getTowerDrainOpKey(creep);
    if (!opKey || !Memory.towerDrainOps || !Memory.towerDrainOps.operations) return null;
    return Memory.towerDrainOps.operations[opKey] || null;
}

function getTowerDrainLane(creep) {
    var op = getTowerDrainOp(creep);
    if (!op || !op.lanes) return null;
    var laneKey = creep.memory.laneSet === true ? creep.memory.laneNumber : creep.memory.laneSet;
    if (laneKey === undefined || laneKey === null) laneKey = creep.memory.laneNumber;
    if (laneKey === undefined || laneKey === null) return null;
    return op.lanes[String(laneKey)] || null;
}

function getTowerDrainRoute(creep, forward) {
    var op = getTowerDrainOp(creep);
    var route = op ? (forward ? op.route : op.routeBack) : null;
    if (Array.isArray(route) && route.length > 0) return route;
    route = forward ? creep.memory.route : creep.memory.routeBack;
    return Array.isArray(route) && route.length > 0 ? route : null;
}

function getTowerDrainPos(creep, field) {
    var lane = getTowerDrainLane(creep);
    if (lane && lane[field]) return lane[field];
    var op = getTowerDrainOp(creep);
    if (field === 'targetPos' && op && op.targetPos) return op.targetPos;
    return creep.memory[field] || null;
}

function getTowerDrainRoom(creep, field) {
    var op = getTowerDrainOp(creep);
    return (op && op[field]) || creep.memory[field];
}

function cleanupTowerDrainCreepMemory(creep) {
    var op = getTowerDrainOp(creep);
    var lane = getTowerDrainLane(creep);
    if (!op || !lane) return;

    delete creep.memory.route;
    delete creep.memory.routeBack;
    delete creep.memory.drainPos;
    delete creep.memory.healPos;
    delete creep.memory.attackEdgePos;
    delete creep.memory.healEdgePos;
    delete creep.memory.attackRestPos;
    delete creep.memory.healRestPos;
    delete creep.memory.targetPos;
    delete creep.memory._pathCache;
}

function getTowerDrainHeapPathCache(creep) {
    if (!global.__towerDrainPathCache) global.__towerDrainPathCache = {};
    return global.__towerDrainPathCache[creep.name] || null;
}

function setTowerDrainHeapPathCache(creep, value) {
    if (!global.__towerDrainPathCache) global.__towerDrainPathCache = {};
    global.__towerDrainPathCache[creep.name] = value;
}

function clearTowerDrainHeapPathCache(creep) {
    if (global.__towerDrainPathCache) delete global.__towerDrainPathCache[creep.name];
    delete creep.memory._pathCache;
}
...
var roleTowerDrain = {
    run: function(creep) {
        cleanupTowerDrainCreepMemory(creep);
        ...
        if (!getTowerDrainRoute(creep, true) || creep.memory.laneSet === undefined) {
        ...
        var lostBodyPart = hasLostBodyPart(creep);
        var lostBodyParts = countLostBodyParts(creep);
        var lostPartRetreat = usesLostPartRetreat(creep);
        ...
        if (lostPartRetreat) {
            // Track TTL/room every tick so onCreepDeath (main.js) can tell an
            // early kill apart from natural expiry and Game.notify about it.
        ...
        var retreatReason = null;
        if (lostPartRetreat) {
            if (lostBodyPart) {
                retreatReason = 'lost body part';
            } else if (creep.room.name === getTowerDrainRoom(creep, 'targetRoom') && hasNearbyHeavyAttacker(creep)) {
                retreatReason = 'nearby heavy attacker';
            }
            if (!retreatReason && creep.room.name === getTowerDrainRoom(creep, 'targetRoom')) {
                retreatReason = getIncomingPartLossThreat(creep);
            }
            if (!retreatReason && creep.memory.variant === 'drainDemolisher' &&
                creep.room.name === getTowerDrainRoom(creep, 'targetRoom') && !findSquadHealer(creep)) {
                retreatReason = 'missing drain-demolisher healer';
            }
        } else if (creep.room.name === getTowerDrainRoom(creep, 'targetRoom') && creep.hits < creep.hitsMax) {
            retreatReason = 'damaged';
        }

        if (retreatReason) {
            if (TOWER_DRAIN_RETREAT_DEBUG &&
                (creep.memory.state !== 'retreating' || creep.memory.retreatReason !== retreatReason)) {
                console.log('[TowerDrain] ' + creep.name + ' retreating: ' + retreatReason +
                            ' hits=' + creep.hits + '/' + creep.hitsMax +
                            ' lostParts=' + lostBodyParts);
            }
            creep.memory.state = 'retreating';
            creep.memory.retreatReason = retreatReason;
            creep.say('🏃');
            this.stateRetreating(creep);
            return;
        }
        
        // Ranged fire runs alongside the melee/work pipeline below — rangedAttack
        // is compatible with heal and dismantle in the same tick.
        var firedRanged = false;
        if (creep.getActiveBodyparts(RANGED_ATTACK) > 0) {
            var rangedHostiles = creep.pos.findInRange(FIND_HOSTILE_CREEPS, 3, {
                filter: function(c) {
                    // Skip creeps protected by a rampart (shots would hit the rampart)
                    var structs = c.pos.lookFor(LOOK_STRUCTURES);
                    for (var i = 0; i < structs.length; i++) {
                        if (structs[i].structureType === STRUCTURE_RAMPART && !structs[i].my) return false;
                    }
                    return true;
                }
            });
            if (rangedHostiles.length > 0) {
                rangedHostiles.sort(function(a, b) { return a.hits - b.hits; });
                creep.rangedAttack(rangedHostiles[0]);
                firedRanged = true;
            }
        }

        if (creep.memory.state !== 'retreating' && creep.getActiveBodyparts(ATTACK) > 0) {
            var nearbyHostiles = creep.pos.findInRange(FIND_HOSTILE_CREEPS, 1);
            if (nearbyHostiles.length > 0) {
                creep.attack(nearbyHostiles[0]);
            } else if (creep.room.name === getTowerDrainRoom(creep, 'targetRoom')) {
        ...
        } else if (creep.memory.state !== 'retreating' && creep.getActiveBodyparts(WORK) > 0) {
            // Normal WORK drainers self-heal when damaged. Lost-part retreat
            // variants keep dismantling through repairable damage.
            if (creep.room.name === getTowerDrainRoom(creep, 'targetRoom') &&
                (creep.hits === creep.hitsMax || (usesLostPartRetreat(creep) && !lostBodyPart))) {
                var nearbyStructures = creep.pos.findInRange(FIND_STRUCTURES, 1, {
                    filter: function(s) { return !s.my; }
                });
                if (nearbyStructures.length > 0) {
                    // Ordered wall/rampart target (drain demolisher): always
                    // chew the designated segment while it stands.
                    var orderedTarget = null;
                    var tp = getTowerDrainPos(creep, 'targetPos');
                    if (tp) {
                        for (var nt = 0; nt < nearbyStructures.length; nt++) {
                            if (nearbyStructures[nt].pos.x === tp.x && nearbyStructures[nt].pos.y === tp.y) {
                                orderedTarget = nearbyStructures[nt];
                                break;
                            }
                        }
                    }
                    if (orderedTarget) {
                        creep.dismantle(orderedTarget);
                    } else {
                        nearbyStructures.sort(function(a, b) {
                            var aWall = a.structureType === STRUCTURE_WALL || a.structureType === STRUCTURE_RAMPART;
                            var bWall = b.structureType === STRUCTURE_WALL || b.structureType === STRUCTURE_RAMPART;
                            if (aWall !== bWall) return (aWall ? 1 : 0) - (bWall ? 1 : 0);
                            if (aWall && bWall) {
                                // Newbie-zone walls have no hits and are indestructible; sort them last
                                return (a.hits || Infinity) - (b.hits || Infinity);
                            }
                            return 0;
                        });
                        creep.dismantle(nearbyStructures[0]);
                    }
                }
            }
        } else if (creep.memory.state !== 'retreating' && !firedRanged && creep.getActiveBodyparts(RANGED_ATTACK) > 0 &&
                   creep.room.name === getTowerDrainRoom(creep, 'targetRoom')) {
            // RA-only variants (no ATTACK/WORK): plink structures when no creep target
            var rangedStructures = creep.pos.findInRange(FIND_STRUCTURES, 3, {
                filter: function(s) { return !s.my; }
            });
            if (rangedStructures.length > 0) {
                creep.rangedAttack(rangedStructures[0]);
            }
        }
    ...
    // ========== SAFE-SQUARE SIDESTEP ==========
    // Safe-room lane squares are single tiles, and targeted drain-demolisher
    // mode packs lanes on adjacent columns, so a square can be squatted by a
    // neighboring worker or passing healer. Pathing at range 0 to an occupied
    // tile fails, leaving the creep loitering wherever it stands (often on
    // someone else's lane square). Instead, wait on a free tile beside the
    // blocked square and step on the tick it frees.

    isLaneSquareBlocked: function(creep, pos) {
        if (creep.room.name !== pos.roomName) return false;
        var tile = new RoomPosition(pos.x, pos.y, pos.roomName);
        var creepsThere = tile.lookFor(LOOK_CREEPS);
        return creepsThere.length > 0 && creepsThere[0].id !== creep.id;
    },

    // Free tile beside the blocked square, perpendicular to the lane axis so
    // the wait spot stays on the depth-1 heal line. Never picks another
    // lane's healRest square (targeted mode packs lanes side by side).
    findSidestepTile: function(creep, pos) {
        var healEdge = getTowerDrainPos(creep, 'healEdgePos');
        var offsets;
        if (healEdge && healEdge.y === pos.y) {
            offsets = [{ dx: 0, dy: -1 }, { dx: 0, dy: 1 }]; // E/W lane, sides vertical
        } else {
            offsets = [{ dx: -1, dy: 0 }, { dx: 1, dy: 0 }]; // N/S lane, sides horizontal
        }
        ...
        var op = Memory.towerDrainOps && Memory.towerDrainOps.operations &&
                 Memory.towerDrainOps.operations[opKey];
        if (op && op.lanes) {
            for (var ln in op.lanes) {
                var hr = op.lanes[ln].healRestPos;
                if (hr && !(hr.x === pos.x && hr.y === pos.y)) {
                    otherLaneTiles[hr.x + ',' + hr.y] = true;
                }
            }
        }
        ...
        for (var i = 0; i < offsets.length; i++) {
            var x = pos.x + offsets[i].dx;
            var y = pos.y + offsets[i].dy;
            if (x < 1 || x > 48 || y < 1 || y > 48) continue;
            if (terrain.get(x, y) === TERRAIN_MASK_WALL) continue;
            if (otherLaneTiles[x + ',' + y]) continue;

            var tile = new RoomPosition(x, y, pos.roomName);
            var creepsThere = tile.lookFor(LOOK_CREEPS);
            if (creepsThere.length > 0 && creepsThere[0].id !== creep.id) continue;

            var blocked = false;
            var structs = tile.lookFor(LOOK_STRUCTURES);
            for (var s = 0; s < structs.length; s++) {
                var st = structs[s].structureType;
                if (st !== STRUCTURE_ROAD && st !== STRUCTURE_CONTAINER &&
                    (st !== STRUCTURE_RAMPART || !structs[s].my)) {
                    blocked = true;
                    break;
                }
            }
            if (!blocked) return { x: x, y: y, roomName: pos.roomName };
        }
        return null;
    },

    // simpleMoveToPos plus the sidestep. Once adjacent (on the sidestep tile),
    // simpleMoveToPos keeps issuing move() toward the square each tick, which
    // succeeds the moment the occupier leaves.
    moveToLaneSquare: function(creep, pos) {
        if (!pos) return;
        var targetPos = new RoomPosition(pos.x, pos.y, pos.roomName);
        if (creep.pos.isEqualTo(targetPos)) return;

        if (creep.room.name === pos.roomName && !creep.pos.isNearTo(targetPos) &&
            this.isLaneSquareBlocked(creep, pos)) {
            var side = this.findSidestepTile(creep, pos);
            if (side) {
                this.simpleMoveToPos(creep, side);
                return;
            }
        }
        this.simpleMoveToPos(creep, pos);
    },

    followRoomRoute: function(creep, forward) {
        var route = getTowerDrainRoute(creep, forward);
        if (!route || route.length === 0) return ERR_NOT_FOUND;
        
        var currentRoom = creep.room.name;
        var goalRoom = forward ? getTowerDrainRoom(creep, 'targetRoom') : getTowerDrainRoom(creep, 'homeRoom');
        ...
        var pathCache = getTowerDrainHeapPathCache(creep) || creep.memory._pathCache;
        ...
        var path;
        if (cacheValid) {
            // moveByPath needs RoomPosition instances; the cache holds plain objects
            path = pathCache.path.map(function(p) {
            ...
            setTowerDrainHeapPathCache(creep, {
                room: currentRoom,
                nextRoom: nextRoom,
                tick: Game.time,
                path: serializedPath
        ...
        if (moveResult !== OK && moveResult !== ERR_TIRED) {
            clearTowerDrainHeapPathCache(creep);
            if (path[0]) {
    ...
    moveToPosition: function(creep, pos) {
        ...
        var safeRoom = getTowerDrainRoom(creep, 'safeRoom');
        var targetRoom = getTowerDrainRoom(creep, 'targetRoom');
        var currentRoom = creep.room.name;
    ...
    stateTraveling: function(creep) {
        ...
        if (creep.room.name === safeRoom) {
            // Drain-demolisher workers can launch once protected. Prefer the
            // paired healer, but a full-health worker can use any parked healer
            // already holding the operation's safe-room heal line.
            if (creep.memory.variant === 'drainDemolisher') {
                var squadHealer = findSquadHealer(creep);
                var protectedByHealer = squadHealer && squadHealer.memory.state === 'parked';
                if (!protectedByHealer && creep.hits === creep.hitsMax) {
                    protectedByHealer = hasParkedDrainDemolisherHealer(creep);
                }
                if (!protectedByHealer) {
                    var waitPos = getTowerDrainPos(creep, 'healRestPos') || getTowerDrainPos(creep, 'healPos');
                    if (waitPos) {
                        var waitRoomPos = new RoomPosition(waitPos.x, waitPos.y, waitPos.roomName);
                        if (!creep.pos.isEqualTo(waitRoomPos)) {
                            this.moveToLaneSquare(creep, waitPos);
                        }
                        creep.say('⌛');
                        return;
                    }
            ...
            var healEdgePos = getTowerDrainPos(creep, 'healEdgePos');
            if (healEdgePos) {
        ...
        var drainPos = getTowerDrainPos(creep, 'drainPos');
        if (!drainPos) {
    ...
    stateDraining: function(creep) {
        if (!usesLostPartRetreat(creep) && creep.hits < creep.hitsMax) {
            creep.memory.state = 'retreating';
            creep.say('🏃');
            return;
        }

        if (hasLostBodyPart(creep)) {
            creep.memory.state = 'retreating';
            creep.say('🏃');
            return;
        }
    ...
    stateRetreating: function(creep) {
        var healPos = getTowerDrainPos(creep, 'healPos');
        var attackEdgePos = getTowerDrainPos(creep, 'attackEdgePos');
        var targetRoom = getTowerDrainRoom(creep, 'targetRoom');
        var safeRoom = getTowerDrainRoom(creep, 'safeRoom');
        ...
        if (creep.room.name === targetRoom) {
            if (attackEdgePos) {
                var edgePos = new RoomPosition(attackEdgePos.x, attackEdgePos.y, attackEdgePos.roomName);
                if (creep.pos.isEqualTo(edgePos)) {
                    if (attackEdgePos.x === 0) creep.move(LEFT);
                    else if (attackEdgePos.x === 49) creep.move(RIGHT);
                    else if (attackEdgePos.y === 0) creep.move(TOP);
                    else if (attackEdgePos.y === 49) creep.move(BOTTOM);
                } else {
        ...
        if (creep.room.name === safeRoom) {
            ...
            this.moveToLaneSquare(creep, healPos);
            creep.say('🏃');
            return;
        }
...
function testTowerDrain(homeRoom, targetRoom, count, preferredEdge, range, extraPart) {
    ...
    console.log('[TowerDrain] === DRY RUN === Planning ' + homeRoom + ' -> ' + targetRoom +
        ' [' + getTowerDrainBodyLabel(options) + '] (no spawning)');
...
function orderBulldozer(homeRoom, targetRoom, count, preferredEdge) {
    var options = getBulldozerOptions(false);

    initMemory();
    if (preferredEdge) preferredEdge = preferredEdge.toUpperCase();
    if (preferredEdge && ['N', 'S', 'E', 'W'].indexOf(preferredEdge) === -1) {
        console.log('[TowerDrain] Invalid preferredEdge: ' + preferredEdge + '. Use N, S, E, or W.');
        return ERR_INVALID_ARGS;
    }

    console.log('[TowerDrain] === BULLDOZER === Ordering ' + homeRoom + ' -> ' + targetRoom +
        ' x' + (count || 1) + ' [' + getTowerDrainBodyLabel(options) + ']');
...
// Resolve and validate the wall/rampart target for a drain-demolisher order.
// Workers only dismantle from the depth-1 drain line, so the target must sit
// within 2 tiles of a room edge; that edge becomes the entry edge.
// Returns { edge, targetPos } or null after logging the reason.
function resolveDrainDemolisherTarget(target, preferredEdge, targetRoom) {
    if (target === undefined || target === null) {
        return { edge: preferredEdge || null, targetPos: null };
    }

    if (typeof target.x !== 'number' || typeof target.y !== 'number' ||
        target.x < 0 || target.x > 49 || target.y < 0 || target.y > 49) {
        console.log('[TowerDrain] Invalid target: expected {x:0-49, y:0-49}, got ' + JSON.stringify(target));
        return null;
    }

    var edges = [];
    if (target.x <= 2) edges.push('W');
    if (target.x >= 47) edges.push('E');
    if (target.y <= 2) edges.push('N');
    if (target.y >= 47) edges.push('S');

    if (edges.length === 0) {
        console.log('[TowerDrain] Target (' + target.x + ',' + target.y + ') is deeper than 2 tiles from every edge. ' +
                    'Drain demolishers hold the depth-1 drain line and cannot reach it.');
        return null;
    }
    ...
    if (edges.length === 1) {
        edge = edges[0];
        if (preferredEdge && preferredEdge !== edge) {
            console.log('[TowerDrain] preferredEdge ' + preferredEdge + ' ignored: target (' +
                        target.x + ',' + target.y + ') is only reachable from the ' + edge + ' edge');
        }
    } else if (preferredEdge && edges.indexOf(preferredEdge) !== -1) {
        edge = preferredEdge;
    } else {
        console.log('[TowerDrain] Target (' + target.x + ',' + target.y + ') is near a corner, reachable from ' +
                    edges.join(' or ') + '. Pass preferredEdge to pick one.');
        return null;
    }

    return { edge: edge, targetPos: { x: target.x, y: target.y, roomName: targetRoom } };
}

function orderDrainDemolisher(homeRoom, targetRoom, count, preferredEdge, target) {
    initMemory();
    if (preferredEdge) preferredEdge = preferredEdge.toUpperCase();
    if (preferredEdge && ['N', 'S', 'E', 'W'].indexOf(preferredEdge) === -1) {
        console.log('[TowerDrain] Invalid preferredEdge: ' + preferredEdge + '. Use N, S, E, or W.');
    ...
    var resolved = resolveDrainDemolisherTarget(target, preferredEdge, targetRoom);
    if (!resolved) return ERR_INVALID_ARGS;
    preferredEdge = resolved.edge;

    count = count || 1;
    if (count > DRAIN_DEMOLISHER_MAX_PAIRS) {
        console.log('[TowerDrain] Drain-demolisher pair count capped at ' + DRAIN_DEMOLISHER_MAX_PAIRS);
        count = DRAIN_DEMOLISHER_MAX_PAIRS;
    }

    var options = getDrainDemolisherOptions(false, resolved.targetPos);

    console.log('[TowerDrain] === DRAIN DEMOLISHER === Ordering ' + homeRoom + ' -> ' + targetRoom +
        ' x' + count + ' pairs [' + getTowerDrainBodyLabel(options) + ']' +
        (resolved.targetPos ? ' target (' + resolved.targetPos.x + ',' + resolved.targetPos.y + ')' : ''));
    ...
    if (home.energyCapacityAvailable < 7500) {
        console.log('[TowerDrain] Cannot start: ' + homeRoom + ' energy capacity ' +
                    home.energyCapacityAvailable + ' < 7500 needed for the 25H/25M healer (RCL8)');
...
function testDrainDemolisher(homeRoom, targetRoom, count, preferredEdge, target) {
    initMemory();
    if (preferredEdge) preferredEdge = preferredEdge.toUpperCase();
    if (preferredEdge && ['N', 'S', 'E', 'W'].indexOf(preferredEdge) === -1) {
        console.log('[TowerDrain] Invalid preferredEdge: ' + preferredEdge + '. Use N, S, E, or W.');
    ...
    count = count || 1;
    if (count > DRAIN_DEMOLISHER_MAX_PAIRS) count = DRAIN_DEMOLISHER_MAX_PAIRS;

    var options = getDrainDemolisherOptions(true, resolved.targetPos);

    console.log('[TowerDrain] === DRAIN DEMOLISHER DRY RUN === Planning ' + homeRoom + ' -> ' + targetRoom +
        ' [' + getTowerDrainBodyLabel(options) + ']' +
        (resolved.targetPos ? ' target (' + resolved.targetPos.x + ',' + resolved.targetPos.y + ')' : '') +
        ' (no spawning)');
...
function testBulldozer(homeRoom, targetRoom, count, preferredEdge) {
    var options = getBulldozerOptions(true);

    console.log('[TowerDrain] === BULLDOZER DRY RUN === Planning ' + homeRoom + ' -> ' + targetRoom +
        ' [' + getTowerDrainBodyLabel(options) + '] (no spawning)');
