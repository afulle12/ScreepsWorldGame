...
function _routeStateKey(creep) {
    return (creep.memory.homeRoom || '?') + '->' +
           (creep.memory.targetRoom || '?') + ':' +
           (creep.memory.mission || 'extensions');
}

function _getRouteState(creep) {
    if (!Memory.remoteSupplierRoutes) Memory.remoteSupplierRoutes = {};

    var key = _routeStateKey(creep);
    var legacy = creep.memory._rs;
    var rs = Memory.remoteSupplierRoutes[key];
    if (rs && rs.lastUsed && Game.time - rs.lastUsed > ROUTE_CACHE_TTL) {
        delete Memory.remoteSupplierRoutes[key];
        rs = null;
    }

    if (!rs) {
        rs = Memory.remoteSupplierRoutes[key] = {
            phase:    'idle',
            route:    null,
            routeBack: null,
            toScan:   [],
    ...
    rs.lastUsed = Game.time;

    if (legacy && legacy !== rs && legacy.route && !rs.route) {
        rs.phase = legacy.phase || 'idle';
        rs.route = legacy.route || null;
        rs.routeBack = legacy.routeBack || (legacy.route ? legacy.route.slice().reverse() : null);
        rs.toScan = legacy.toScan || [];
        rs.pending = legacy.pending || null;
        rs.blocked = legacy.blocked || [];
        rs.attempts = legacy.attempts || 0;
    }

    creep.memory._rs = { key: key };
    return rs;
}

function _resetRouteState(creep) {
    var key = _routeStateKey(creep);
    if (Memory.remoteSupplierRoutes) delete Memory.remoteSupplierRoutes[key];
...
function _pickRandomEdgeTile(roomName, edgeDir) {
    var walkable = util.edgeWalkableTiles(roomName, edgeDir);
