...
function getScoutState(creep) {
    if (!Memory.scoutState) Memory.scoutState = {};
    const state = Memory.scoutState[creep.name] || (Memory.scoutState[creep.name] = {});
    ...
    for (let i = 0; i < migratedFields.length; i++) {
        const field = migratedFields[i];
        if (creep.memory[field] !== undefined && state[field] === undefined) {
            state[field] = creep.memory[field];
        }
        if (creep.memory[field] !== undefined) delete creep.memory[field];
    }
    return state;
}

function rememberPathRoom(creep, state, fallbackRoom) {
    if (!state.pathTaken) state.pathTaken = [fallbackRoom || creep.room.name];
    if (state.pathTaken[state.pathTaken.length - 1] !== creep.room.name) {
        state.pathTaken.push(creep.room.name);
    }
    return state.pathTaken;
}
...
const roleScout = {
    ...
    orderAutonomousInterShardScout: function(preferredShard) {
        ...
        const result = spawnManager.spawnCustomCreep(spawn, body, newName, memory);
        if (result === OK) {
    ...
    orderCheckRoom: function(spawnRoomName, destinationRoomName) {
        ...
        const body = [MOVE];
        const memory = { role: 'scout', task: 'checkRoom', destinationRoom: destinationRoomName, spawnRoom: spawnRoomName };
        const result = spawnManager.spawnCustomCreep(spawn, body, newName, memory);
        if (result === OK) {
    ...
    spawnPathfinderScout: function(missionId) {
        ...
        const body = [MOVE];
        const memory = { role: 'scout', task: 'pathfinder', missionId: missionId, originRoom: mission.origin, destinationRoom: mission.destination };
        const result = spawnManager.spawnCustomCreep(spawn, body, newName, memory);
        if (result === OK) {
    ...
    handleDeadCreeps: function() {
        ...
        for (const name in Memory.creeps) {
            ...
            if (Memory.scoutState) delete Memory.scoutState[name];
            delete Memory.creeps[name];
        }
    },
    ...
    run: function(creep) {
        ...
        const scoutState = getScoutState(creep);
        ...
        if (creep.memory.lastRoom !== creep.room.name) {
            ...
            if (!creep.memory.backtracking) {
                delete scoutState.localBlacklist;
            }
    ...
    travelToRoom: function(creep, targetRoomName) {
        const scoutState = getScoutState(creep);
        if (!scoutState.route || scoutState.routeTarget !== targetRoomName) {
            delete scoutState.route;
            ...
            if (scoutState.localBlacklist && Object.keys(scoutState.localBlacklist).length > 0) log(`   - Temp local blacklist: [${Object.keys(scoutState.localBlacklist).join(', ')}]`);

            const route = Game.map.findRoute(creep.room.name, targetRoomName, {
                routeCallback: (roomName) => {
                    if (creep.memory.task === 'pathfinder') {
                        if (missionBlacklist[roomName]) return Infinity;
                        if (scoutState.localBlacklist && scoutState.localBlacklist[roomName]) return Infinity;
                    } else {
            ...
            scoutState.route = route;
            scoutState.routeTarget = targetRoomName;
        }

        const route = scoutState.route;
        if (route && route.length > 0) {
            ...
            if (route.length > 0) {
                ...
                if (exit) {
                    ...
                    if (!scoutState.localBlacklist) {
                        scoutState.localBlacklist = {};
                    }
                    scoutState.localBlacklist[failedExitRoom] = true;
    ...
    runPathfinderTask: function(creep) {
        ...
        if (creep.room.name === destination) {
            const pathTaken = rememberPathRoom(creep, scoutState, creep.memory.originRoom);
            const pathString = pathTaken.join(' → ');
            mission.status = 'success';
            mission.foundPath = pathTaken;
        ...
        rememberPathRoom(creep, scoutState, creep.memory.originRoom);
    ...
    runCheckRoomTask: function(creep) {
        ...
        if (creep.room.name === destination) {
            if (!creep.memory.notificationSent) {
                const pathTaken = rememberPathRoom(creep, scoutState, creep.memory.spawnRoom);
                const pathString = pathTaken.join(' → ');
            ...
            rememberPathRoom(creep, scoutState, creep.memory.spawnRoom);
            const travelResult = this.travelToRoom(creep, destination);
            if (travelResult === ERR_NO_PATH) {
                if (!creep.memory.notificationSent) {
    ...
    runAutonomousTask: function(creep) {
        ...
        if (creep.memory.lastRoom !== creep.room.name) {
            ...
            delete scoutState.route;
            if (!scoutState.visitedRooms) scoutState.visitedRooms = {};
            scoutState.visitedRooms[creep.room.name] = Game.time;
    ...
    assignTargetRoom: function(creep) {
        ...
        for (const dir in exits) {
            ...
            const recentlyVisited = scoutState.visitedRooms[roomName] && (Game.time - scoutState.visitedRooms[roomName] < 500);
        ...
        for (const dir in exits) {
            ...
            if (!this.isRoomDangerous(roomName)) {
                const lastVisit = scoutState.visitedRooms[roomName] || 0;
