...
module.exports = {
        ...
        switch (creep.memory.state) {
            ...
            default:
                ...
                if (creep.pos.getRangeTo(powerSpawn) > 3) {
                    creep.moveTo(powerSpawn, { reusePath: 10 });
                }
                break;
        }
    },
        ...
        for (var res in creep.store) {
            if (creep.store[res] > 0) {
                ...
                if (result === ERR_NOT_IN_RANGE) {
                    creep.moveTo(returnTarget, { visualizePathStyle: { stroke: '#ff00ff' }, reusePath: 10 });
                }
    ...
    deliverEnergy: function(creep, powerSpawn) {
        ...
        if (result === ERR_NOT_IN_RANGE) {
            creep.moveTo(powerSpawn, { visualizePathStyle: { stroke: '#ffaa00' }, reusePath: 10 });
    ...
    deliverPower: function(creep, powerSpawn) {
        ...
        if (result === ERR_NOT_IN_RANGE) {
            creep.moveTo(powerSpawn, { visualizePathStyle: { stroke: '#ff0000' }, reusePath: 10 });
        } else if (result === ERR_FULL) {
            if ((creep.store[RESOURCE_ENERGY] || 0) > 0) {
                creep.memory.state = 'deliver_energy';
                creep.say('🔋 fill E');
