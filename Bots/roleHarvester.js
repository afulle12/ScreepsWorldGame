...
const SUSPENDED_HARVESTER_RENEW_TTL = 1400;
...
var roleHarvester = {
    run: function(creep) {
        ...
        // ═══════════════════════════════════════════════════════
        // SUSPENDED-ROOM RENEWAL FAST PATH
        // When the room is suspended and energy is still above the
        // harvester floor, the harvester's normal logic may not renew
        // (sources are full, TTL is high, etc.). Override behavior so
        // the harvester explicitly seeks out an adjacent spawn and
        // renews itself before TTL drops too low.
        // ═══════════════════════════════════════════════════════
        if (roomSuspender.canRunSuspendedHarvester(creep) &&
            creep.ticksToLive < SUSPENDED_HARVESTER_RENEW_TTL) {

            var sbt = state.structuresByType || {};
            var spawns = sbt[STRUCTURE_SPAWN] || [];
            if (spawns.length > 0) {
                // Pick the nearest non-spawning spawn
                var target = null;
                var bestRange = Infinity;
                for (var si = 0; si < spawns.length; si++) {
                    var sp = spawns[si];
                    if (!sp || sp.spawning) continue;
                    var r = creep.pos.getRangeTo(sp);
                    if (r < bestRange) { bestRange = r; target = sp; }
                }
                if (target) {
                    if (creep.pos.isNearTo(target)) {
                        ...
                        creep.moveTo(target, { reusePath: 20, maxRooms: 1 });
                        return;
                    }
                }
            }
