...
function towerRepairLimit(structure, queuedTarget, defenseMax, maxHeal) {
  if (!structure) return 0;
  var limit = structure.hitsMax ? Math.floor(structure.hitsMax * TOWER_REPAIR_MAX_RATIO) : structure.hitsMax;
  if (queuedTarget && queuedTarget > 0) limit = Math.min(limit, queuedTarget);
  if (!maxHeal && (structure.structureType === STRUCTURE_WALL || structure.structureType === STRUCTURE_RAMPART) && defenseMax) {
    limit = Math.min(limit, defenseMax);
  }
  return limit;
}

// Per-structure 5.5M tower cap for ramparts protecting non-critical
// buildings (tower, lab, link, factory, observer).
var PROTECTED_RAMPART_CAP_BY_TYPE = repairManager.constants.TOWER_REPAIR_CAP_BY_TYPE;

// Used only to find the dominant structure type within range 1 of a rampart.
var RAMPART_TARGETS = repairManager.constants.RAMPART_TARGETS;

function getProtectedRampartCap(rampart, rs) {
  if (!rampart || !rs || !rs.structuresByType) return null;
  var maxType = null;
  for (var type in RAMPART_TARGETS) {
    var arr = rs.structuresByType[type] || [];
    for (var i = 0; i < arr.length; i++) {
      var s = arr[i];
      if (s && rampart.pos.getRangeTo(s.pos) <= 1) {
        if (!maxType || RAMPART_TARGETS[type] > RAMPART_TARGETS[maxType]) {
          maxType = type;
        }
      }
    }
  }
  if (maxType && PROTECTED_RAMPART_CAP_BY_TYPE[maxType]) {
    return PROTECTED_RAMPART_CAP_BY_TYPE[maxType];
  }
  return null;
}

function effectiveTowerRepairLimit(structure, queuedTarget, defenseMax, maxHeal, rs) {
  var limit = towerRepairLimit(structure, queuedTarget, defenseMax, maxHeal);
  if (structure && structure.structureType === STRUCTURE_RAMPART) {
    var cap = getProtectedRampartCap(structure, rs);
    if (cap !== null && limit > cap) limit = cap;
  }
  return limit;
}
...
function getManagerHealTarget(roomName, tick) {
  var plan = Memory.repairPlan && Memory.repairPlan[roomName];
  if (!plan || !plan.hq || !plan.hq.length) return null;
  var healTick = plan.hqt || plan.t || plan.tick || 0;
  if (tick - healTick > MANAGER_PLAN_FRESH_TICKS) return null;
  for (var i = 0; i < plan.hq.length; i++) {
    var entry = plan.hq[i];
    var name = Array.isArray(entry) ? entry[0] : (entry.name || entry.id);
    var pc = name && Game.powerCreeps && Game.powerCreeps[name];
    if (!pc || !pc.ticksToLive || !pc.room || pc.room.name !== roomName) continue;
    if (pc.hits < pc.hitsMax) return pc;
  }
  return null;
}

// Reconcile pending tower fire with tombstones from the previous tick.
// We record the targets fired on this tick; if a tombstone appears next
// tick with a matching deathTime and creep name, the kill is attributed
// to our towers. This covers both NPC and player creeps because
// FIND_TOMBSTONES returns both.
function reconcilePendingTowerKills(tick) {
  if (!Memory.stats) Memory.stats = {};
  if (typeof Memory.stats.kills !== 'number') Memory.stats.kills = 0;

  var pending = heap.pendingKills;
  for (var roomName in pending) {
    var entry = pending[roomName];
    if (!entry || entry.tick >= tick) continue;

    var room = Game.rooms[roomName];
    if (!room) { delete pending[roomName]; continue; }

    var tombstones = room.find(FIND_TOMBSTONES);
    for (var i = 0; i < tombstones.length; i++) {
      var tomb = tombstones[i];
      if (tomb.deathTime !== entry.tick) continue;
      if (entry.targets[tomb.creep.name]) {
        Memory.stats.kills++;
      }
    }
...
// roomName  – room owning the towers; used to record fired targets for
//             tombstone-based kill attribution on the next tick
function assignTowersToTargets(targets, towers, roomName) {
  var towerIdx = 0;
  ...
  for (var ti = 0; ti < targets.length && towerIdx < tCount; ti++) {
    ...
    if (isLowThreatCreep(target)) {
      ...
      if (towers[towerIdx].attack(target) === OK) {
        if (!fired) fired = {};
        fired[target.name] = true;
      }
      towerIdx++;
    } else {
  ...
  if (roomName && fired) {
    heap.pendingKills[roomName] = { tick: Game.time, targets: fired };
  }
}
...
function getUrgentTargets(roomName, rs, tick) {
  ...
  if (uc && tick - uc.tick < 3) {
    ...
    for (var i = 0; i < uc.targets.length; i++) {
      ...
      if (obj && obj.hits < towerRepairLimit(obj, null, null, true)) valid.push(uc.targets[i]);
    }
    uc.targets = valid;
    return valid;
  }
...
function runTowers() {
  ...
  // Reconcile tombstone kills for tower fire from the previous tick
  reconcilePendingTowerKills(tick);
  ...
  for (var ri = 0; ri < rLen; ri++) {
    ...
    if (roomSuspender.isSuspended(roomName)) continue;
    ...
    if (hostiles && hostiles.length > 0) {
      ...
      if (confirmed.length > 0) {
        ...
        if (healer && nonHealer && tCount >= 2) {
          ...
          assignTowersToTargets([healer, nonHealer], towers, roomName);
        } else {
        ...
        confirmed.sort(function(a, b) {
          ...
          assignTowersToTargets(confirmed, towers, roomName);
        }
        continue;
      }
    }
    ...
    var roomMaxHeal = !!(repairManager && repairManager.isMaxHeal && repairManager.isMaxHeal(roomName));
    var unleashed = !!Memory.towers[roomName] || roomMaxHeal;
    var spreadTowerTargets = false;
    var spreadPlan = Memory.repairPlan && Memory.repairPlan[roomName];
    if (spreadPlan) {
      spreadTowerTargets = !!((spreadPlan.tp && spreadPlan.tp.s === 1) ||
        (spreadPlan.towerPolicy && spreadPlan.towerPolicy.spreadTargets));
    }
    ...
    if (primaryE >= TOWER_ENERGY_RESERVE) {
      ...
      var healT = getManagerHealTarget(roomName, tick);
      var needHealScan = false;

      if (!healT && hCache && hCache.id) {
    ...
    if (urgentTargets.length > 0) {
      if (unleashed) {
        var urgIdx = 0;
        var usedUrgentTargets = spreadTowerTargets ? {} : null;
        for (var ui = 0; ui < tCount && urgIdx < urgentTargets.length; ui++) {
          ...
          while (spreadTowerTargets && urgTarget && usedUrgentTargets[urgTarget.id] && urgIdx < urgentTargets.length) {
            urgIdx++;
            urgTarget = Game.getObjectById(urgentTargets[urgIdx]);
          }
          if (urgTarget && urgTarget.hits < towerRepairLimit(urgTarget, null, null, true)) {
            tw.repair(urgTarget);
            if (ui === 0) primaryE -= 10;
            if (spreadTowerTargets) {
              usedUrgentTargets[urgTarget.id] = true;
              urgIdx++;
            } else if (urgTarget.hitsMax > 0 &&
        ...
        if (urgTarget && urgTarget.hits < towerRepairLimit(urgTarget, null, null, true) && primaryE >= TOWER_ENERGY_RESERVE) {
    ...
    if (state === 'scan') {
      ...
      if (!roomMaxHeal && managerPlan && compactQueue && tick - managerTick <= MANAGER_PLAN_FRESH_TICKS) {
        var expandedQueue = [];
        for (var mq = 0; mq < compactQueue.length; mq++) {
          var entry = compactQueue[mq];
          if (Array.isArray(entry)) {
            expandedQueue.push({ id: entry[0], pri: entry[1], val: entry[2], border: entry[3], target: entry[4] });
          } else {
        ...
        if (!managerPolicy && managerPlan.tp) {
          managerPolicy = { repairTowerLimit: managerPlan.tp.l, reservePerTower: managerPlan.tp.r, maxHeal: managerPlan.tp.m === 1, spreadTargets: managerPlan.tp.s === 1 };
        } else if (managerPolicy) {
          if (managerPolicy.maxHeal === undefined && managerPlan.tp && managerPlan.tp.m === 1) managerPolicy.maxHeal = true;
          if (managerPolicy.spreadTargets === undefined && managerPlan.tp && managerPlan.tp.s === 1) managerPolicy.spreadTargets = true;
        }
      ...
      if (sbt) {
        for (var sti = 0; sti < REPAIR_TYPES.length; sti++) {
          ...
          var isD = isW || isR;
          if (isD && !allowW && !roomMaxHeal) continue;
          ...
          for (var di = 0; di < arr.length; di++) {
            try {
              ...
              var sM = s.hitsMax;
              var repairLimit = effectiveTowerRepairLimit(s, null, defenseMax, roomMaxHeal, rs);
              if (sH >= repairLimit) continue;
              if (isD && !roomMaxHeal && sH >= defenseMax) continue;
              ...
              queue.push({ id: s.id, pri: pri, val: v, border: isBorderPos(s.pos) ? 1 : 0, target: repairLimit });
    ...
    if (state === 'repair') {
      ...
      while (q && qi < q.length) {
        ...
        var queueMaxHeal = roomMaxHeal || !!(rp.towerPolicy && rp.towerPolicy.maxHeal);
        if (candidate && candidate.hits < effectiveTowerRepairLimit(candidate, q[qi].target, defenseMax, queueMaxHeal, rs)) {
          var cst = candidate.structureType;
          var isDefense = (cst === STRUCTURE_WALL || cst === STRUCTURE_RAMPART);
          if (isDefense && !queueMaxHeal && candidate.hits >= defenseMax) {
            qi++; continue;
          }
          repT = candidate;
          break;
      ...
      if (!repT) {
        ...
        if (unleashed && rp.towerPolicy && rp.towerPolicy.spreadTargets) {
          var usedTargets = {};
          var spreadIdx = qi;
          for (var sti = 0; sti < tCount && repCount < maxRepairTowers; sti++) {
            var stw = towers[sti];
            var stE = (sti === 0) ? primaryE : stw.store[RESOURCE_ENERGY];
            if (stE < TOWER_ENERGY_RESERVE) continue;

            var spreadTarget = null;
            while (q && spreadIdx < q.length) {
              var spreadCandidate = Game.getObjectById(q[spreadIdx].id);
              var spreadMaxHeal = roomMaxHeal || !!(rp.towerPolicy && rp.towerPolicy.maxHeal);
              if (spreadCandidate && !usedTargets[spreadCandidate.id] && spreadCandidate.hits < effectiveTowerRepairLimit(spreadCandidate, q[spreadIdx].target, defenseMax, spreadMaxHeal, rs)) {
                var spreadType = spreadCandidate.structureType;
                var spreadDefense = (spreadType === STRUCTURE_WALL || spreadType === STRUCTURE_RAMPART);
                if (!spreadDefense || spreadMaxHeal || spreadCandidate.hits < defenseMax) {
                  spreadTarget = spreadCandidate;
                  break;
                }
              }
              spreadIdx++;
            ...
            stw.repair(spreadTarget);
            usedTargets[spreadTarget.id] = true;
            repCount++;
            spreadIdx++;
          }
        } else if (unleashed) {
