...
function getRefPrice(compound) {
  var avg = pricing.getAvg48h(compound);
  return avg !== null ? avg : 1.0;
}
  ...
  for (var compound in order.boosts) {
    ...
    if (obReq2 && obReq2.remaining > 0) {
      ...
      if (stallTicks >= BUY_ORDER_STALL_TICKS && !order.buyOrderIds[compound]) {
        var buyPrice = Math.round(refPrice * PRICE_MULT_ORDER * 1000) / 1000; // server keeps 3-decimal prices
        var orderAmt = obReq2.remaining;

        var orderFee = pricing.FEE * buyPrice * orderAmt;
        var res = creditLedger.available() >= orderFee ? Game.market.createOrder({
          type: ORDER_BUY, resourceType: compound,
          price: buyPrice, totalAmount: orderAmt, roomName: roomName
        }) : ERR_NOT_ENOUGH_RESOURCES;

        if (res === OK) {
          creditLedger.commit(orderFee);
