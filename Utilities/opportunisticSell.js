 ...
 * Best external ask from marketPricing's book (dust- and own-room-filtered,
 * cached per tick), falling back to the 48h volume-weighted average.
 *
 * @returns {number} Energy price in credits per unit
 */
function getEnergyMarketPrice() {
    var book = pricing.getBook(RESOURCE_ENERGY);
    if (book.bestAsk !== null) return book.bestAsk;
    var avg = pricing.getAvg48h(RESOURCE_ENERGY);
    return avg !== null ? avg : 0.01;
}
...
function processAccountResource(key_i, request, requests, myRooms) {
    ...
    function attemptAccountDeal(order) {
        ...
        // resources — `order` may come from the shared util.marketOrders()
    ...
    if (!attempted) {
        var orders = util.marketOrders(request.resourceType, ORDER_BUY).filter(function(order) {
