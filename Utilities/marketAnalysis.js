...
(function registerMarketAnalysisGlobal() {
  ...
  var util = require('util'); // shared getAllOrders() snapshot only — no cycle back to this module
  ...
  function computeActualBuyPrice(resourceType) {
    var orders = util.marketOrders(resourceType, ORDER_BUY);
  ...
  function computeActualSellPrice(resourceType) {
    var orders = util.marketOrders(resourceType, ORDER_SELL);
    var myRooms = getMyRooms(), valid = [];
    for (var i = 0; i < orders.length; i++) {
      var o = orders[i];
      if (!o || typeof o.remainingAmount !== 'number' || o.remainingAmount < 1000) continue;
  ...
  function buildBook(resourceType) {
    var buys  = util.marketOrders(resourceType, ORDER_BUY);
    var sells = util.marketOrders(resourceType, ORDER_SELL);
