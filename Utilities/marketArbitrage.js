 ...
 *   marketArbitrage.setMaxExposure(50000)   -- change max in-flight units per resource
 *   marketArbitrage.setMinSelfArbProfit(1)  -- change min absolute credit profit for self-arb
 *   marketArbitrage.setFloorSweepDebug(true) -- toggle verbose floor-sweep logging
 *   marketArbitrage.addFloorSweepBan('Z')   -- ban a resource from floor sweeps
 *   marketArbitrage.removeFloorSweepBan('Z') -- lift a floor-sweep ban
 *   marketArbitrage.setSellGhostFailThreshold(3) -- change sell-ghost fail threshold for groups
 *   marketArbitrage.setGroupStaleTicks(200) -- change group stale-warning age
 *
 */
...
var BAD_BUY_ORDER_COOLDOWN = 100;   // Heap-only blacklist for buy orders that reject sell deals
...
// Low-bucket floor: when the CPU bucket is below 5k, the adaptive scanner
// stretches its minimum interval from 2 -> 3 ticks so marketArbitrage runs
// at most every third tick instead of every other tick.
var LOW_BUCKET_FLOOR_INTERVAL = 3;
var LOW_BUCKET_THRESHOLD      = 5000;

function getMinScanInterval() {
    return (Game.cpu.bucket < LOW_BUCKET_THRESHOLD) ? LOW_BUCKET_FLOOR_INTERVAL : MIN_SCAN_INTERVAL;
}
...
if (typeof Memory.marketArbitrage.scanInterval === 'number') {
    SCAN_INTERVAL = Math.max(getMinScanInterval(), Math.min(MAX_SCAN_INTERVAL, Memory.marketArbitrage.scanInterval));
}
...
// Re-verify an order against live market data immediately before dealing.
// Books and opportunities are computed from the shared util.marketSnapshot()
// (via marketPricing.getRawOrders), which can be up to util.ORDER_TTL ticks
// stale; deal() must never fire on an order that has since been filled,
// cancelled, or shrunk below the planned amount. Skipping is safe — the
// failure path runs and the op re-scans on a later tick.
function orderStillLive(orderId, amount) {
    var o = Game.market.getOrderById(orderId);
    return !!(o && o.amount >= amount);
}

function getBadBuyOrders() {
    var bad = global.__marketArbBadBuyOrders;
    if (!bad) bad = global.__marketArbBadBuyOrders = {};
    return bad;
}

function isBadBuyOrder(orderId) {
    if (!orderId) return false;
    var bad = getBadBuyOrders();
    var tick = bad[orderId];
    if (tick === undefined) return false;
    if (Game.time - tick > BAD_BUY_ORDER_COOLDOWN) {
        delete bad[orderId];
        return false;
    }
    return true;
...
function markBadBuyOrder(orderId, result) {
    if (!orderId) return;
    if (result !== ERR_INVALID_ARGS && result !== ERR_FULL) return;
    getBadBuyOrders()[orderId] = Game.time;
}

function pruneBadBuyOrders() {
    var bad = getBadBuyOrders();
    for (var id in bad) {
        if (Game.time - bad[id] > BAD_BUY_ORDER_COOLDOWN) delete bad[id];
    }
}
...
function getCostBasisFloor(entry) {
    if (!entry || typeof entry.sellPrice !== 'number' || entry.sellPrice <= 0) return null;
    return (typeof entry.costBasis === 'number' && entry.costBasis >= entry.sellPrice)
        ? entry.costBasis
        : entry.sellPrice;
}

function netSellPricePerUnit(price, amount, fromRoom, toRoom, terminal, energyPrice) {
    if (amount <= 0) return -Infinity;
    var sellEnergy = calcEffectiveEnergyCost(amount, fromRoom, toRoom, terminal);
    return price - (sellEnergy * energyPrice / amount);
}

function terminalUsedThisTick(roomName, localUsed) {
    return !!(localUsed && localUsed[roomName]) || (util.wasTerminalUsed && util.wasTerminalUsed(roomName));
}

function markTerminalIntent(roomName, localUsed) {
    if (localUsed) localUsed[roomName] = true;
    if (util.markTerminalUsed) util.markTerminalUsed(roomName);
}

function describeDealFailure(result, roomName, resourceType, amount, order, terminal) {
    var orderId = order && order.id ? order.id : '?';
    var orderRoom = order && order.roomName ? order.roomName : '?';
    var orderAmount = order ? (order.remainingAmount || order.amount || 0) : 0;
    var liveOrder = orderId !== '?' ? Game.market.getOrderById(orderId) : null;
    var liveOrderAmount = liveOrder ? (liveOrder.remainingAmount || liveOrder.amount || 0) : 0;
    var terminalStock = terminal && terminal.store ? (terminal.store[resourceType] || 0) : 0;
    var terminalEnergy = terminal && terminal.store ? (terminal.store[RESOURCE_ENERGY] || 0) : 0;
    var energyCost = (terminal && order && order.roomName) ? calcEffectiveEnergyCost(amount, roomName, order.roomName, terminal) : '?';
    return 'result=' + result + ', resource=' + resourceType + ', amount=' + amount +
        ', order=' + orderId + ', orderRoom=' + orderRoom + ', orderAmount=' + orderAmount +
        ', liveOrderAmount=' + liveOrderAmount +
        ', terminalStock=' + terminalStock + ', terminalEnergy=' + terminalEnergy +
        ', energyCost=' + energyCost + ', cooldown=' + (terminal ? terminal.cooldown : '?');
}
...
/**
 * Remove group bookkeeping that can no longer make progress. Group members are
 * represented by active ops; once those ops are gone, buffered leftovers are
 * tracked separately and the group should not keep warning forever.
 */
function cleanupGroups(ops) {
    var groups = Memory.marketArbitrage.groups;
    if (!groups) return;

    var activeGroupKeys = {};
    for (var roomName in ops) {
        var op = ops[roomName];
        if (op && op.groupKey) activeGroupKeys[op.groupKey] = true;
    }
    ...
    for (var gk in groups) {
        ...
        if (expected <= 0 || sellDone >= expected || !activeGroupKeys[gk]) {
            delete groups[gk];
        }
    }
}
...
/** Scan-local cache for marketPricing.getAvg48h(). Cleared every run(). */
var _historyCache = {};
...
/**
 * Get the 48h volume-weighted average price cap for a resource, via
 * marketPricing.getAvg48h. _historyCache memoizes the result within a scan.
...
function getHistoryCap(resource, multiplier) {
    if (_historyCache[resource] === undefined) {
        _historyCache[resource] = pricing.getAvg48h(resource);
    }
...
function buildOrderBooks(myRooms, oppBuyRequests) {
    ...
    var myOrderIds = Game.market.orders || {};
    var buysByRes  = {};
    var sellsByRes = {};
    ...
    for (var i = 0; i < allOrders.length; i++) {
        ...
        // Own orders: the roomName check below misses account-resource orders
        // (no roomName), so filter by order id -- otherwise account arb can
        // "profitably" deal against our own pixel/cpuUnlock buy orders.
        if (myOrderIds[o.id]) continue;
        ...
        if (o.type === ORDER_BUY) {
            ...
            if (isBadBuyOrder(o.id)) continue;
    ...
    // or an internal opportunistic buy request. Resources whose only buyers
    // were filtered out (e.g. by MIN_BUY_ORDER_PRICE) must not be scanned as
    // if they still had a market.
    var viable = {};
    ...
    for (var resource in books.sellsByRes) {
        ...
        if (buys) {
            for (var bi = 0; bi < buys.length && buyDepth.length < BUY_DEPTH; bi++) {
                var cb = buys[bi];
                if (isBadBuyOrder(cb.id)) continue;
        ...
        for (var s = 0; s < sells.length; s++) {
            ...
            // The book snapshot can be up to util.ORDER_TTL ticks stale; clamp
            // to the live order or a filled/shrunk sell gets re-picked and
            // re-failed every scan tick until the snapshot refreshes.
            var liveSell = Game.market.getOrderById(sell.id);
            var liveSellAmt = liveSell ? liveSell.amount : 0;
            if (liveSellAmt <= 0) continue;
            var sellAvailable = Math.min(sell.amount, liveSellAmt) - sellCommitted;
...
function evaluateCandidate(cand, terminal, energyPrice, numIdleTerminals, activeResourceTypes) {
    ...
    var credits         = creditLedger.available();
...
function findFloorSweepOpportunity(terminal, books, energyPrice, committedSellAmounts, resourceExposure, bufferedResources, ownSellResources, activeResourceTypes) {
    ...
    for (var resource in books.sellsByRes) {
        ...
        for (var s = 0; s < sells.length; s++) {
            ...
            var sellCommitted = committedSellAmounts[sell.id] || 0;
            // Same staleness clamp as buildArbCandidates: verify against the
            // live order so dead/shrunk sells aren't re-attempted every scan.
            var liveSell = Game.market.getOrderById(sell.id);
            var liveSellAmt = liveSell ? liveSell.amount : 0;
            if (liveSellAmt <= 0) continue;
            var sellAvailable = Math.min(sell.amount || 0, liveSellAmt) - sellCommitted;
            if (sellAvailable <= 0) continue;
...
function processOwnSellOrders(books, energyPrice, myRooms, committedBuyAmounts, oppBuyRequests, usedTerminals) {
    ...
    for (var pRoom in pending) {
        if (roomSuspender.shouldAvoidRoomWork(pRoom)) continue;
        var p = pending[pRoom];
    ...
    for (var orderId in myOrders) {
        ...
        if (terminalUsedThisTick(roomName, usedTerminals)) continue;
        ...
        if (oppList) {
            ...
            for (var bi = 0; bi < buyOrders.length; bi++) {
                var bo = buyOrders[bi];
                if (isBadBuyOrder(bo.id)) continue;
                ...
                if (sfNetPerUnit * SELF_FULFILL_ADVANTAGE > bestExtNetPerUnit) {
                    var ownSfResult = terminal.send(order.resourceType, sfAmt, opp.roomName);
                    if (ownSfResult === OK) {
                        ...
                        markTerminalIntent(roomName, usedTerminals);
        ...
        var selfDealResult = orderStillLive(best.buyOrder.id, best.amount) ? Game.market.deal(best.buyOrder.id, best.amount, roomName) : ERR_INVALID_ARGS;
        if (selfDealResult === OK) {
            pending[roomName] = {
            ...
            markBadBuyOrder(best.buyOrder.id, selfDealResult);
            console.log('[Arbitrage] SELF-ARB deal failed in ' + roomName + ': ' +
                describeDealFailure(selfDealResult, roomName, order.resourceType, best.amount, best.buyOrder, terminal));
        }
    }
}
...
function processAccountOpSellAndScan(books, committedBuyAmounts, committedSellAmounts, resourceExposure, myRooms, ownSellResources) {
    ...
    if (op && op.state === 'ready_to_sell') {
        ...
        if (sellAmt <= 0) {
            ...
            if (!buyOrder || buyRemaining <= 0) {
                ...
                // Same staleness guard as the terminal replacement search: the
                // book snapshot can be up to util.ORDER_TTL ticks stale, so
                // verify candidates against the live market before accepting.
                var found = null, foundEff = 0;
                for (var ri = 0; ri < repls.length; ri++) {
                    var c = repls[ri];
                    if (isBadBuyOrder(c.id)) continue;
                    var liveRepl = Game.market.getOrderById(c.id);
                    var liveReplAmt = liveRepl ? (liveRepl.remainingAmount || liveRepl.amount || 0) : 0;
                    var cEff = liveReplAmt - (committedBuyAmounts[c.id] || 0);
                    if (cEff <= 0) continue;
                    if (c.price > op.sellPrice) { found = c; foundEff = cEff; break; }
                }
                if (found) {
                    buyOrder = found;
                    buyRemaining = foundEff;
            ...
            var accountDealResult = (!isBadBuyOrder(buyOrder.id) && orderStillLive(buyOrder.id, dealAmt)) ? Game.market.deal(buyOrder.id, dealAmt) : ERR_INVALID_ARGS;
            if (accountDealResult === OK) {
                ...
                markBadBuyOrder(buyOrder.id, accountDealResult);
    ...
    if (!op) {
        ...
        if (orderStillLive(opp.sellOrder.id, opp.amount) && Game.market.deal(opp.sellOrder.id, opp.amount) === OK) {
            creditLedger.commit(opp.amount * opp.sellOrder.price);
 ...
 */
function processPendingBuyVerify(op, ops, roomName) {
 ...
 */
function processPendingVerify(op, ops, roomName) {
    ...
    if (tx) {
        ...
        op.amount -= actual;
        op.soldTotal = (op.soldTotal || 0) + actual;
        ...
        if (op.amount <= 0) {
            if (grp) {
                // op.soldTotal: a room may confirm across several buyers/chunks;
                // record the cumulative amount, not just the final tx.
                grp.sellConfirmedRooms[roomName] = { amount: op.soldTotal, profit: op.profit };
 ...
 * receiving the goods.
 */
function processBufferedEntries(buffered, books, oppBuyRequests, energyPrice, committedBuyAmounts, usedTerminals) {
    for (var bi = buffered.length - 1; bi >= 0; bi--) {
        ...
        if (buf.pendingVerify) {
            ...
            if (tx) {
                ...
                if (buf.pendingVerify && typeof buf.pendingVerify.basis === 'number' &&
                    tx.order && typeof tx.order.price === 'number' && tx.order.price < buf.pendingVerify.basis) {
                    console.log('[Arbitrage] WARNING: BUFFER verified sale price below basis: ' +
                        tx.order.price + ' < ' + buf.pendingVerify.basis.toFixed(3) + ' in ' + roomName);
                }
        ...
        // One deal/send per terminal per tick across ALL passes -- a second
        // deal() returns OK but fails in intent processing, producing a ghost.
        if (terminalUsedThisTick(roomName, usedTerminals)) continue;
        ...
        var costBasis = getCostBasisFloor(buf);
        if (costBasis === null) {
            console.log('[Arbitrage] BUFFER dropping ' + buf.amount + ' ' + buf.resourceType +
                ' in ' + roomName + ' -- invalid cost basis. Cannot verify profitability.');
            buffered.splice(bi, 1); continue;
        }
        ...
        // -- Self-fulfill --
        // Use true costBasis as the floor so internal sends do not consume buffered
        // inventory below its all-in acquisition cost.
        ...
        if (oppList) {
            for (var oi = 0; oi < oppList.length; oi++) {
                ...
                if (sfAmt <= 0) continue;
                var sfNetPerUnit = netSellPricePerUnit(opp.maxPrice, sfAmt, roomName, opp.roomName, terminal, energyPrice);
                if (sfNetPerUnit <= minAcceptable) continue;
                if (terminalUsedThisTick(roomName, usedTerminals)) continue;
                var sfResult = terminal.send(buf.resourceType, sfAmt, opp.roomName);
                if (sfResult === OK) {
                    console.log('[Arbitrage] BUFFER self-fulfill ' + sfAmt + ' ' + buf.resourceType + ' -> ' + opp.roomName +
                        ' (net ' + sfNetPerUnit.toFixed(3) + ', basis ' + costBasis.toFixed(3) + ')');
                    markTerminalIntent(roomName, usedTerminals);
                    opp.remaining -= sfAmt;
                    if (Memory.opportunisticBuy && Memory.opportunisticBuy.requests && Memory.opportunisticBuy.requests[opp.key]) {
                        var or = Memory.opportunisticBuy.requests[opp.key];
        ...
        var netRevenuePerUnit = netSellPricePerUnit(bestBuy.price, fillAmt, roomName, bestBuy.roomName, terminal, energyPrice);
        ...
        netRevenuePerUnit = netSellPricePerUnit(bestBuy.price, capped, roomName, bestBuy.roomName, terminal, energyPrice);
        if (netRevenuePerUnit <= minAcceptable) continue;

        if (terminalUsedThisTick(roomName, usedTerminals)) continue;
        var bufferDealResult = orderStillLive(bestBuy.id, capped) ? Game.market.deal(bestBuy.id, capped, roomName) : ERR_INVALID_ARGS;
        if (bufferDealResult === OK) {
            buf.pendingVerify = { tick: Game.time, orderId: bestBuy.id, amount: capped, price: bestBuy.price, basis: costBasis, net: netRevenuePerUnit };
            markTerminalIntent(roomName, usedTerminals);
            ...
            console.log('[Arbitrage] BUFFER deal() OK ' + capped + ' ' + buf.resourceType +
                ...
                ' (net ' + netRevenuePerUnit.toFixed(3) + ', basis ' + costBasis.toFixed(3) + ') | Verifying...');
        } else {
            markBadBuyOrder(bestBuy.id, bufferDealResult);
            console.log('[Arbitrage] BUFFER sell failed in ' + roomName + ': ' +
                describeDealFailure(bufferDealResult, roomName, buf.resourceType, capped, bestBuy, terminal));
        }
    }
}
 ...
 */
function processPendingSell(terminal, op, ops, roomName, energyPrice, oppBuyRequests, books, usedTerminals) {
    if (terminal.cooldown > 0) return;
    if (terminalUsedThisTick(roomName, usedTerminals)) return;
    var availEnergy = terminal.store[RESOURCE_ENERGY] || 0;
    var grp = op.groupKey && Memory.marketArbitrage.groups[op.groupKey];
    var costBasis = getCostBasisFloor(op);
    if (costBasis === null) {
        console.log('[Arbitrage] Dropping op in ' + roomName + ' -- invalid cost basis for ' + op.resourceType);
        delete ops[roomName];
        return;
    }
    var minAcceptable = costBasis * (1 + MIN_PROFIT_MARGIN);
    ...
    if (oppList) {
        for (var oi = 0; oi < oppList.length; oi++) {
            ...
            var sfResult = terminal.send(op.resourceType, sfAmt, opp.roomName);
            if (sfResult === OK) {
                ...
                op.amount -= sfAmt;
                op.soldTotal = (op.soldTotal || 0) + sfAmt;
                if (op.amount <= 0) {
                    if (grp) {
                        ...
                        grp.sellConfirmedRooms[roomName] = { amount: op.soldTotal, profit: 0 };
                        if (Object.keys(grp.sellConfirmedRooms).length >= groupExpectedCount(grp)) {
                            logSellConfirmed(grp);
                            delete Memory.marketArbitrage.groups[op.groupKey];
                        }
    ...
    if (depth && depth.length && op.buyDepthIndex < depth.length) {
        ...
        for (var depthTries = 0; depthTries < BUY_DEPTH; depthTries++) {
            ...
            if (isBadBuyOrder(depthEntry.order.id)) {
                depth[di].amount = 0;
            ...
            var depthNetPerUnit = netSellPricePerUnit(depthEntry.order.price, capped, roomName, depthEntry.order.roomName, terminal, energyPrice);
            if (depthNetPerUnit <= minAcceptable) {
                depth[di].amount = 0;
                op.buyDepthIndex = di + 1;
                continue;
            }

            var depthDealResult = orderStillLive(depthEntry.order.id, capped) ? Game.market.deal(depthEntry.order.id, capped, roomName) : ERR_INVALID_ARGS;
            if (depthDealResult === OK) {
                ...
                if (grp) {
                    ...
                    console.log('[Arbitrage] deal() OK ' + capped + ' ' + op.resourceType +
                        ...
                        ' (net ' + depthNetPerUnit.toFixed(3) + ', basis ' + costBasis.toFixed(3) +
                        ', depth ' + (di + 1) + '/' + depth.length + ') | Verifying...');
                }
                return;
            } else {
                markBadBuyOrder(depthEntry.order.id, depthDealResult);
                ...
                console.log('[Arbitrage] deal() failed for depth buyer ' + depthEntry.order.id + ' in ' + roomName + ': ' +
                    describeDealFailure(depthDealResult, roomName, op.resourceType, capped, depthEntry.order, terminal));
                depth[di].amount = 0;
                op.buyDepthIndex = di + 1;
                continue;
            }
    ...
    if (!buyOrder || buyRemaining <= 0) {
        ...
        // The book snapshot can be up to util.ORDER_TTL ticks stale. Verify each
        // candidate against the live market before accepting it, or a dead order
        // gets re-picked by every pending_sell op on every scan tick until the
        // snapshot refreshes (deal() skipped -> "Sell failed" -> buyOrderId
        // nulled -> same replacement found again).
        var found = null, foundLiveAmt = 0;
        for (var ri = 0; ri < repls.length; ri++) {
            var c = repls[ri];
            if (isBadBuyOrder(c.id)) continue;
            var liveRepl = Game.market.getOrderById(c.id);
            var liveReplAmt = liveRepl ? (liveRepl.remainingAmount || liveRepl.amount || 0) : 0;
            if (liveReplAmt <= 0) continue;
            var cAmt = Math.min(op.amount, liveReplAmt);
            var cNetPerUnit = netSellPricePerUnit(c.price, cAmt, roomName, c.roomName, terminal, energyPrice);
            if (cNetPerUnit > minAcceptable) { found = c; foundLiveAmt = liveReplAmt; break; }
        }
        if (found) {
            buyOrder = found; buyRemaining = foundLiveAmt;
            ...
            if (oppList) {
                for (var fi = 0; fi < oppList.length; fi++) {
                    var fb = oppList[fi];
                    if (fb.roomName === roomName) continue;
                    var fbAmt = capByEnergy(Math.min(op.amount, fb.remaining), roomName, fb.roomName, availEnergy, terminal);
                    if (fbAmt <= 0) continue;
                    var fbNetPerUnit = netSellPricePerUnit(fb.maxPrice, fbAmt, roomName, fb.roomName, terminal, energyPrice);
                    if (fbNetPerUnit <= minAcceptable) continue;
                    var fbResult = fbAmt > 0 ? terminal.send(op.resourceType, fbAmt, fb.roomName) : ERR_INVALID_ARGS;
                    if (fbResult === OK) {
                        console.log('[Arbitrage] FALLBACK self-fulfill ' + fbAmt + ' ' + op.resourceType + ' -> ' + fb.roomName +
                            ' (net ' + fbNetPerUnit.toFixed(3) + ', basis ' + costBasis.toFixed(3) + ')');
                        markTerminalIntent(roomName, usedTerminals);
                        ...
                        op.amount -= fbAmt;
                        op.soldTotal = (op.soldTotal || 0) + fbAmt;
                        if (op.amount <= 0) {
                            if (grp) {
                                grp.sellConfirmedRooms[roomName] = { amount: op.soldTotal, profit: 0 };
                                if (Object.keys(grp.sellConfirmedRooms).length >= groupExpectedCount(grp)) {
    ...
    var netPerUnit = netSellPricePerUnit(buyOrder.price, capped, roomName, buyOrder.roomName, terminal, energyPrice);
    if (netPerUnit <= minAcceptable) {
        console.log('[Arbitrage] Sell blocked in ' + roomName + ': net ' + netPerUnit.toFixed(3) +
            ' below basis floor ' + minAcceptable.toFixed(3) + ' for ' + op.resourceType);
        op.buyOrderId = null;
        return;
    }

    var dealResult = (!isBadBuyOrder(buyOrder.id) && orderStillLive(buyOrder.id, capped)) ? Game.market.deal(buyOrder.id, capped, roomName) : ERR_INVALID_ARGS;
    if (dealResult === OK) {
        op.state = 'pending_verify';
        ...
        if (grp) {
            ...
            console.log('[Arbitrage] deal() OK ' + capped + ' ' + op.resourceType +
                ' -> ' + buyOrder.roomName + ' @ ' + buyOrder.price +
                ' (net ' + netPerUnit.toFixed(3) + ', basis ' + costBasis.toFixed(3) + ') | Verifying...');
        }
    } else {
        markBadBuyOrder(buyOrder.id, dealResult);
        console.log('[Arbitrage] Sell failed in ' + roomName + ': ' +
            describeDealFailure(dealResult, roomName, op.resourceType, capped, buyOrder, terminal));
...
function adaptScanInterval() {
    ...
    var cpuStats = require('memoryManager').heap.cpuStats;
    if (!cpuStats || !cpuStats.history || cpuStats.history.length === 0) return;

    var history = cpuStats.history;
    var avg = history.reduce(function(sum, val) { return sum + val; }, 0) / history.length;

    var prev = SCAN_INTERVAL;
    var minInterval = getMinScanInterval();
    if (avg < CPU_TARGET) {
        SCAN_INTERVAL = Math.max(minInterval, SCAN_INTERVAL - 1);
    } else if (avg > CPU_TARGET) {
        SCAN_INTERVAL = Math.min(MAX_SCAN_INTERVAL, SCAN_INTERVAL + 1);
    }
    if (SCAN_INTERVAL < minInterval) SCAN_INTERVAL = minInterval;
...
function run() {
    ...
    if (!isScanTick) {
        // After a global reset the caches are unpopulated until the first scan
        // tick; _cachedEnergyPrice = 0 would make replacement-search profit
        // checks ignore energy cost entirely. Wait for the first scan instead.
        if (!_cachedBooks) return;
        var interUsedTerminals = {};
        for (var psRoom in ops) {
            var psOp = ops[psRoom];
            if (!psOp || psOp.state !== 'pending_sell') continue;
            if (roomSuspender.shouldAvoidRoomWork(psRoom)) continue;
            ...
            processPendingSell(psTerminal, psOp, ops, psRoom, _cachedEnergyPrice, _cachedOppBuyRequests, _cachedBooks, interUsedTerminals);
        }
        return;
    }
    ...
    for (var rn in Game.rooms) {
        ...
        if (roomSuspender.shouldAvoidRoomWork(rn)) continue;
    ...
    // usedTerminals: rooms whose terminal has already issued a deal()/send()
    // this tick, shared across ALL passes. A second deal from the same
    // terminal returns OK but fails in intent processing (cooldown set by the
    // first), which the verify step then misreads as a ghost.
    var usedTerminals = {};
    var idleTerminals = [];
    var terminalRooms = {};
    ...
    for (var t = 0; t < terminals.length; t++) {
        ...
        if (op) {
            ...
            if (op.state === 'pending_buy_verify')  processPendingBuyVerify(op, ops, roomName);
            else if (op.state === 'pending_verify') processPendingVerify(op, ops, roomName);
            else if (op.state === 'pending_sell')   processPendingSell(terminal, op, ops, roomName, energyPrice, oppBuyRequests, books, usedTerminals);
        } else if (terminal.cooldown <= 0) {
            idleTerminals.push(terminal);
        }
    }

    // Ops whose room dropped out of the terminals list (suspended, downgraded
    // below RCL6, terminal destroyed) are never processed and never expire --
    // they inflate resourceExposure indefinitely. Warn periodically.
    if (Game.time % 100 === 0) {
        for (var stRoom in ops) {
            if (terminalRooms[stRoom]) continue;
            var stOp = ops[stRoom];
            console.log('[Arbitrage] WARNING: op in ' + stRoom + ' (' + stOp.amount + ' ' +
                stOp.resourceType + ', ' + stOp.state + ') has no usable terminal and is not being processed.' +
                " Use marketArbitrage.cancel('" + stRoom + "') to clear it.");
        }
    }
    ...
    if (buffered) {
        for (var bt = buffered.length - 1; bt >= 0; bt--) {
            var b = buffered[bt];
            if (b && roomSuspender.shouldAvoidRoomWork(b.roomName)) continue;
    ...
    if (hasBuffered) processBufferedEntries(buffered, books, oppBuyRequests, energyPrice, committedBuyAmounts, usedTerminals);
    ...
    for (var si = 0; si < idleTerminals.length; si++) {
        ...
        // Skip terminals that already dealt/sent this tick in an earlier pass
        // (self-arb, buffered sale) -- a second deal would silently fail and
        // produce a phantom op / bogus ghost blacklist entry.
        if (terminalUsedThisTick(sTerm.room.name, usedTerminals)) continue;
        var sBest = 0;
        var sBestOpp = null;
        var sBestCand = null;
    ...
    for (var i = 0; i < rankedTerminals.length; i++) {
        ...
        if (!bestOpp) {
            ...
            if (sweepOpp && !(ghosts && ghosts[sweepOpp.sellOrder.id])) {
                if (orderStillLive(sweepOpp.sellOrder.id, sweepOpp.amount) && Game.market.deal(sweepOpp.sellOrder.id, sweepOpp.amount, tRoom) === OK) {
                    creditLedger.commit(sweepOpp.amount * sweepOpp.sellOrder.price);
                    markTerminalIntent(tRoom, usedTerminals);
        ...
        if (orderStillLive(bestOpp.sellOrder.id, bestOpp.amount) && Game.market.deal(bestOpp.sellOrder.id, bestOpp.amount, tRoom) === OK) {
            creditLedger.commit(bestOpp.amount * bestOpp.sellOrder.price);
            markTerminalIntent(tRoom, usedTerminals);
