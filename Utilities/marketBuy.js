...
var marketBuyer = {
    ...
    // Cancel fulfilled orders (remainingAmount === 0)
    cleanupFulfilledOrders: function() {
        ...
        for (var orderId in myOrders) {
            ...
            // Only clean up BUY orders with 0 remaining
            if (order.type === ORDER_BUY && order.remainingAmount === 0) {
                var result = Game.market.cancelOrder(orderId);
                if (result === OK) {
                    cancelledCount++;
                    console.log('[MarketBuy] Auto-cancelled fulfilled order: ' + orderId);
                }
            }
        }
    ...
    // Cleanup expired/completed gather ops (callable from your main loop).
    run: function() {
    ...
    marketBuy: function(roomName, resourceType, amount, price) {
        ...
        // Clean up any fulfilled orders before creating new ones
