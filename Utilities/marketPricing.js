 ...
 *
 * Key properties:
 *  - The filtered book (getBook/getAvg48h/etc.) is rebuilt at most once per
 *    tick. The underlying raw order list comes from util.marketSnapshot(),
 *    a cache shared across every market module with its own TTL (default
 *    ~30 ticks) — getAllOrders() is one of the most expensive API calls,
 *    and most consumers here don't need fresher data than that.
...
// getAllOrders() is expensive. Delegates to util.marketSnapshot(), which is
// shared across ALL market modules (not just this one) and keyed by TTL
// rather than tick equality, since many callers run on staggered tick
// offsets and would otherwise never actually share a per-tick cache.
// Callers must not mutate the returned array — it's a shared reference.
