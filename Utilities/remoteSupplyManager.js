...
global.remoteSupply = function(sourceRoom, recipientRoom, missions) {
    ...
    if (!playerName) {
        ...
        if (pm && pm.state && pm.state[recipientRoom]) {
            pm.state[recipientRoom].t = 0;
        }
        lines.push('  playerMonitor scan timestamp reset - rescan queued for next cycle.');
    }
...
function _findPlayerForRoom(roomName) {
    var pm = Memory.playerMonitor;
    var reg = Memory.roomRegistry;
    if (!pm || !pm.players || !reg || !reg.rooms || !reg.rooms[roomName]) return null;
    var playerName = reg.rooms[roomName].o;
    return pm.players[playerName] ? playerName : null;
}

function _getRoomState(roomName) {
    var pm = Memory.playerMonitor;
    if (!pm || !pm.state) return null;
    return pm.state[roomName] || null;
}
