...
// Resets at midnight Pacific Time. Console: financeReport(), financeReport('rooms')
...
const dailyFinance = {
    ...
    report: function (mode) {
        ...
        const sign = net >= 0 ? '+' : '';
        var trackedProfit = fin.totalIncome - fin.totalExpenses;
        ...
        L.push('Time: ' + timeStr +
               ...
               ' | Balance: ' + dailyFinance._fmtNum(currentBalance));
        L.push('');
        ...
        L.push('  Net Credit Change:' + (sign + dailyFinance._fmtNum(net)).padStart(36));
        L.push('');
        L.push('  Income:' + dailyFinance._fmtNum(fin.totalIncome).padStart(47));
        L.push('  Expenses:' + dailyFinance._fmtNum(fin.totalExpenses).padStart(45));
        L.push('  Tracked Profit:' + dailyFinance._fmtNum(trackedProfit).padStart(39));
        if (delta >= 0) {
        ...
        // ── Income ──
        L.push('--- SALES / ARBITRAGE PERFORMANCE ---');
        ...
        if (expenseKeys.length === 0) {
            ...
            L.push('  ' +
                'Resource'.padEnd(26) + '| ' +
                'Credits'.padStart(12) + ' | ' +
                '% of Non-Resale Purchases'.padStart(13));
            L.push('  ' + '-'.repeat(57));
            for (let i = 0; i < expenseKeys.length; i++) {
        ...
        const hasAnomalies = nullOrder && (nullOrder.incoming || nullOrder.outgoing);
        ...
        // ── Report health ──
        if (hasAnomalies || (fin.hourlyChecks && fin.hourlyChecks.length > 0)) {
            L.push('');
            L.push('--- REPORT HEALTH ---');
            if (hasAnomalies) {
    ...
    _checkMidnightReset: function () {
        ...
        if (Memory.dailyFinance.dateString !== today) {
            ...
            // Reset tower kill counter at the same midnight PT boundary
